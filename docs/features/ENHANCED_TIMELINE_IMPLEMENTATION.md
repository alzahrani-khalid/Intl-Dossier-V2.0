...
```css
@media (max-width: 767px) /* Mobile phones */ @media (max-width: 1169px) /* Tablets */ @media (min-width: 1170px); /* Desktop */
```
...
function MyTimelineComponent() {
  const { events, isLoading, isFetchingNextPage, hasNextPage, fetchNextPage, error } =
    useUnifiedTimeline({
