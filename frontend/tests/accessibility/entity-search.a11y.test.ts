...
test.describe('Entity Search Accessibility', () => {
  ...
  test('should support keyboard navigation with Tab key', async ({ page }) => {
    ...
    for (let i = 0; i < focusableCount + 1; i++) {
      ...
      const isFocusInDialog = await currentFocus.evaluate(
        (el, dialogEl) => {
  ...
  test('should support screen reader text for icon-only buttons', async ({ page }) => {
    ...
    const closeButton = searchDialog.locator(
  ...
  test('should maintain focus within modal during Tab navigation (focus trap)', async ({
    page,
  }) => {
    // Open entity search dialog
    const addLinkButton = page
