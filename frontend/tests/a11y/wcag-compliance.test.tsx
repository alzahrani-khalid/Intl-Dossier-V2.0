...
import { DesignProvider } from '../../src/design-system/DesignProvider'
...
const TestWrapper = ({ children }: { children: React.ReactNode }) => (
  <DesignProvider>
    <LanguageProvider>
      <DesignComplianceProvider>{children}</DesignComplianceProvider>
    </LanguageProvider>
  </DesignProvider>
)
...
describe('WCAG 2.1 Level AA Compliance', () => {
  describe('ResponsiveCard Accessibility', () => {
    ...
    it('should have proper ARIA attributes for collapsible cards', async () => {
      ...
      const { container, getByRole } = render(
        <TestWrapper>
          <ResponsiveCard title="Collapsible Card" collapsible={true} defaultCollapsed={false}>
            <p>Card content</p>
          </ResponsiveCard>
        </TestWrapper>,
      )
  ...
  describe('ResponsiveTable Accessibility', () => {
    ...
    it('should have no accessibility violations in table view', async () => {
      const { container } = render(
        <TestWrapper>
          <ResponsiveTable data={mockData} columns={columns} mobileView="table" />
        </TestWrapper>,
      )
    ...
    it('should have no violations in card view', async () => {
      const { container } = render(
        <TestWrapper>
          <ResponsiveTable data={mockData} columns={columns} mobileView="card" />
        </TestWrapper>,
      )
  ...
  describe('ValidationBadge Accessibility', () => {
    ...
    it('should provide proper status information', async () => {
      const { container } = render(
        <TestWrapper>
          <ValidationBadge componentName="TestComponent" showDetails={true} />
        </TestWrapper>,
      )

      const badge =
        container.querySelector('[role="status"]') || container.querySelector('[aria-live]')
        ...
        <TestWrapper>
          <div>
            <ResponsiveCard title="Contrast Test">
              ...
              <button className="bg-primary text-primary-foreground">Button Text</button>
            </ResponsiveCard>
          </div>
        </TestWrapper>,
      )
  ...
  describe('Touch Targets', () => {
    it('should have minimum touch target size', async () => {
      const { container } = render(
        <TestWrapper>
          <div>
            <button className="min-w-[44px] min-h-[44px] p-2">Touch Target</button>
            <ResponsiveCard>
