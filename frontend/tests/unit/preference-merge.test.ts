...
export const parseStoredPreferences = (stored: string | null): Partial<Preferences> | null => {
...
const isValidTheme = (theme: any): theme is Preferences['theme'] => {
  return theme === 'chancery' || theme === 'situation'
}
...
describe('Preference Merging Logic Unit Tests', () => {
  describe('mergePreferences', () => {
    ...
    it('should merge single source with defaults', () => {
      const result = mergePreferences({ theme: 'situation' })
    ...
    it('should override earlier sources with later ones', () => {
      const source1 = { theme: 'situation', colorMode: 'dark' }
      const source2 = { theme: 'chancery' } // Should override blueSky
  ...
  describe('mergeWithPriority', () => {
    it('should respect priority order: user > supabase > localStorage > system > defaults', () => {
      const systemPrefs = { theme: 'situation', colorMode: 'dark' }
      const localStoragePrefs = { theme: 'chancery', language: 'ar' }
      const supabasePrefs = { colorMode: 'light' }
      const userOverrides = { theme: 'situation' }

      const result = mergeWithPriority(systemPrefs, localStoragePrefs, supabasePrefs, userOverrides)

      expect(result).toEqual({
        theme: 'situation', // From userOverrides
    ...
    it('should work without user overrides', () => {
      ...
      const supabasePrefs = { theme: 'situation' }

      const result = mergeWithPriority(systemPrefs, localStoragePrefs, supabasePrefs)

      expect(result).toEqual({
        theme: 'situation', // From supabase
