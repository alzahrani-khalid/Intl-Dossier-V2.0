...
describe('applyTokens — DOM side effects', () => {
  ...
  it('writes every token entry to document.documentElement', () => {
    const tokens = buildTokens({ direction: 'linear', mode: 'light', density: 'comfortable' })
  ...
  it('cleanup restores a token that had a prior value', () => {
    ...
    const tokens = buildTokens({ direction: 'linear', mode: 'dark', density: 'dense' })
    const cleanup = applyTokens(tokens)
  ...
  it('calling applyTokens twice lets the second cleanup restore the first set', () => {
    // --bg differs across modes (light #ffffff vs dark #010102), so it is the
    // discriminating token here — the accent is brand-invariant across modes.
    const first = buildTokens({ direction: 'linear', mode: 'light', density: 'comfortable' })
    const second = buildTokens({ direction: 'linear', mode: 'dark', density: 'dense' })

    applyTokens(first)
    const cleanupSecond = applyTokens(second)

    const root = document.documentElement
    expect(root.style.getPropertyValue('--bg')).toBe(second['--bg'])

    cleanupSecond()
    expect(root.style.getPropertyValue('--bg')).toBe(first['--bg'])
  })
