...
describe('EscalationDialog Component', () => {
  ...
  it('should show loading state when isLoading is true', () => {
    render(<EscalationDialog {...defaultProps} isLoading={true} />)
  ...
  describe('Mobile-First Responsive Layout', () => {
    it('should apply mobile-first padding classes', () => {
      const { container } = render(<EscalationDialog {...defaultProps} />)
  ...
  describe('RTL Support', () => {
    ...
    it('should use logical properties for padding instead of physical sides', () => {
      const { container } = render(<EscalationDialog {...defaultProps} />)

      // Check for logical properties in container
      const dialogContent = container.querySelector('[role="dialog"]')
      const classes = dialogContent?.className || ''
      const physicalStartPadding = new RegExp(`\\bp${'l'}-`)
      const physicalEndPadding = new RegExp(`\\bp${'r'}-`)

      // Should not have pl- or pr- classes
      expect(classes).not.toMatch(physicalStartPadding)
      expect(classes).not.toMatch(physicalEndPadding)

      // Should have ps- or pe- classes instead
      expect(classes.match(/\b(ps-|pe-)/)).toBeTruthy()
    })

    it('should use logical text alignment', () => {
      render(<EscalationDialog {...defaultProps} />)

      const reasonTextarea = screen.getByPlaceholderText(/explain why/i)
      const physicalTextAlign = ['text', 'left'].join('-')
      expect(reasonTextarea).toHaveClass('text-start')
      expect(reasonTextarea).not.toHaveClass(physicalTextAlign)
    })
  })
