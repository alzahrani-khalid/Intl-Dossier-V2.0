...
test.describe('After-Action Create Flow', () => {
  ...
  test('should create after-action with draft → publish → verify tasks created', async ({
    page,
  }) => {
    ...
    await page.fill(
      'textarea[name="decisions[0].description_en"]',
      'Approved budget increase for Q2',
    )
    await page.fill(
      'textarea[name="decisions[0].description_ar"]',
      'تمت الموافقة على زيادة الميزانية للربع الثاني',
    )
    await page.selectOption('select[name="decisions[0].category"]', 'financial')

    await page.click('button:has-text("Add Decision")')
    await page.fill(
      'textarea[name="decisions[1].description_en"]',
      'Decided to postpone Phase 3 timeline',
    )
    await page.fill(
      'textarea[name="decisions[1].description_ar"]',
      'تقرر تأجيل الجدول الزمني للمرحلة الثالثة',
    )
    ...
    await page.fill(
      'textarea[name="commitments[0].description_en"]',
      'Prepare revised budget proposal',
    )
    await page.fill(
      'textarea[name="commitments[0].description_ar"]',
      'إعداد مقترح الميزانية المعدل',
    )
    ...
    await page.click('button:has-text("Add Commitment")')
    await page.fill(
      'textarea[name="commitments[1].description_en"]',
      'Update project timeline document',
    )
    await page.fill(
      'textarea[name="commitments[1].description_ar"]',
      'تحديث وثيقة الجدول الزمني للمشروع',
    )
    ...
    await page.click('button:has-text("Add Commitment")')
    await page.fill(
      'textarea[name="commitments[2].description_en"]',
    ...
    await page.fill(
      'textarea[name="risks[0].mitigation_en"]',
    ...
    await page.fill(
      'textarea[name="follow_ups[0].description_en"]',
      'Schedule follow-up meeting in 2 weeks',
    )
    await page.fill(
      'textarea[name="follow_ups[0].description_ar"]',
    ...
    await page.waitForResponse(
      (response) =>
        response.url().includes(`/after-action/publish/${afterActionId}`) &&
        response.status() === 200,
    )
    ...
    const task1 = page.locator(
      'text=Prepare revised budget proposal >> xpath=ancestor::div[@role="row"]',
    )
    await expect(task1.locator('text=John Doe')).toBeVisible()
    await expect(task1.locator('text=2025-01-30')).toBeVisible()

    const task2 = page.locator(
      'text=Update project timeline document >> xpath=ancestor::div[@role="row"]',
    )
    await expect(task2.locator('text=Jane Smith')).toBeVisible()
    await expect(task2.locator('text=2025-02-05')).toBeVisible()

    const task3 = page.locator(
      'text=Coordinate with finance department >> xpath=ancestor::div[@role="row"]',
    )
  ...
  test('should support auto-save draft every 30 seconds', async ({ page }) => {
    ...
    await expect(page.locator('textarea[name="purpose"]')).toHaveValue(
  ...
  test('should be mobile-responsive with touch-friendly targets', async ({ page }) => {
    ...
    for (let i = 0; i < count - 1; i++) {
      ...
      if (box1 && box2) {
        ...
        if (gap < 100) {
          // Only check vertically adjacent buttons
