...
test.describe('SLA Tracking', () => {
  ...
  test('T082: Task with deadline in 2 hours shows warning indicator after 75% elapsed', async ({
    page,
  }) => {
  ...
  test('T083: Breached SLA shows red indicator and appears in "Breached SLA" filter', async ({
    page,
  }) => {
    const now = new Date()
    ...
    const filteredIndicator = page.locator(
      `article:has-text("${breachedTaskTitle}") [data-testid="sla-indicator"]`,
    )
