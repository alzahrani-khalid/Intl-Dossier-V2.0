 ...
 *
 * Prerequisites:
 * - Multiple users must exist (TEST_USER_EMAIL + test contributor)
...
async function createTask(page: Page, title: string) {
  ...
  const createButton = page
    .locator('button:has-text("New Task"), button:has-text("Create Task"), a:has-text("New Task")')
    .first()
  ...
  await page.fill(
    ...
    'Test task for contributor tests',
  )
...
test.describe('Contributors - User Story 2', () => {
  ...
  test('T052: should add contributor and display in contributor list', async ({ page }) => {
    ...
    await expect(
      page.locator(`h1:has-text("${taskTitle}"), h2:has-text("${taskTitle}")`),
    ).toBeVisible()
    ...
    await expect(
      page.locator(
        '[data-testid="add-contributor-dialog"], [role="dialog"]:has-text("Add Contributor")',
      ),
    ...
    const firstUserResult = page
      .locator('[data-testid="user-search-result"], [role="option"]')
      .first()
    ...
    if (await roleSelect.isVisible()) {
      ...
      await page
        .locator('option:has-text("Helper"), [role="option"]:has-text("Helper")')
        .first()
        .click()
    } else {
    ...
    await page.waitForResponse(
      (response) =>
        (response.url().includes('/contributors') ||
          response.url().includes('/contributors-add')) &&
        [200, 201].includes(response.status()),
      { timeout: 10000 },
    )
    ...
    const contributorAvatar = page
      .locator('[data-testid="contributor-avatar"], .contributor-avatar')
      .first()
    ...
    const firstContributor = page
      .locator('[data-testid="contributor-item"], .contributor-item')
      .first()
  ...
  test('T053: should remove contributor from active list', async ({ page }) => {
    ...
    await expect(page.locator('[role="dialog"]:has-text("Add Contributor")')).toBeVisible({
      timeout: 5000,
    })
    ...
    const roleHelper = page
      .locator('button:has-text("Helper"), [data-testid="role-helper"]')
      .first()
    ...
    if (await confirmDialog.isVisible({ timeout: 1000 })) {
      ...
      await page.click(
        'button:has-text("Remove"), button:has-text("Confirm"), button:has-text("Yes")',
      )
    }
    ...
    await page.waitForResponse(
      (response) =>
        (response.url().includes('/contributors') ||
          response.url().includes('/contributors-remove')) &&
    ...
    const activeContributors = page.locator(
      '[data-testid="contributors-list"] [data-testid="contributor-item"]',
    )
   ...
   */
  test('T054: should display task in "Tasks I Contributed To" filter', async ({
    page,
    browser,
  }) => {
    // Step 1: Task owner (test user) creates a task and adds a contributor
    ...
    const contributedFilterButton = page
      .locator(
    ...
    if (await contributedFilterButton.isVisible({ timeout: 3000 })) {
      ...
      const myContributorBadge = page.locator(
        '[data-testid="contributor-item"]:has-text("You"), .contributor-item:has-text("Me")',
      )
...
test.describe('Contributors - Mobile & RTL Compliance', () => {
  ...
  test('should support RTL layout for Arabic in contributors UI', async ({ page }) => {
    ...
    const langSelector = page.locator(
      '[data-testid="language-selector"], button:has-text("English")',
    )
