...
test.describe('Link Migration Workflow', () => {
  ...
  test('should migrate all links when converting intake to position with 100% success rate', async ({
    page,
  }) => {
    ...
    const convertButton = page.locator(
      'button:has-text("Convert to Position"), button:has-text("تحويل إلى موقف")',
    )
    ...
    const linkTypes = await migratedLinkCards
      .locator('[data-testid="link-type-badge"]')
      .allTextContents()
  ...
  test('should preserve link metadata during migration (notes, created_at)', async ({ page }) => {
    ...
    const linkManagerSection = page.locator('[data-testid="entity-link-manager"]')
    const addLinkButton = linkManagerSection
      .locator('button:has-text("Add Link"), button:has-text("إضافة رابط")')
      .first()
    await addLinkButton.click()

    const searchDialog = page.locator('[role="dialog"]')
    const searchInput = searchDialog.locator(
    ...
    // Add notes to link
    const notesInput = searchDialog.locator(
      'textarea[placeholder*="notes"], textarea[placeholder*="ملاحظات"]',
    )
    if (await notesInput.isVisible({ timeout: 1000 })) {
      await notesInput.fill('Important bilateral relationship for trade policy')
    }

    const createLinkButton = searchDialog.locator(
      'button:has-text("Create Link"), button:has-text("إنشاء رابط")',
    )
...
async function createLink(
  page: any,
  entityName: string,
  linkType: 'primary' | 'related' | 'requested' | 'mentioned' | 'assigned_to',
) {
  const linkManagerSection = page.locator('[data-testid="entity-link-manager"]')
  const addLinkButton = linkManagerSection
    .locator('button:has-text("Add Link"), button:has-text("إضافة رابط")')
