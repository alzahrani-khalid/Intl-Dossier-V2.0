...
test.describe('Reverse Lookup Workflow', () => {
  ...
  test('should filter intakes by link type', async ({ page }) => {
    ...
    const linkTypeFilter = relatedIntakesSection
      .locator('[data-testid="link-type-filter"], select[name="link-type"], [role="combobox"]')
      .first()
  ...
  test('should display clearance-filtered results', async ({ page }) => {
    ...
    const errorMessage = page.locator(
      '[role="alert"]:has-text("clearance"), [role="alert"]:has-text("صلاحية")',
    )
