...
test.describe('Assignment Details View', () => {
  ...
  test('should display aging indicator with correct color based on days waiting', async ({
    page,
  }) => {
  ...
  test('should show last reminder timestamp if reminder was sent', async ({ page }) => {
    ...
    const assignmentWithReminder = page
      .locator('[data-testid="assignment-row"][data-has-reminder="true"]')
      .first()
    ...
    if (await assignmentWithReminder.isVisible()) {
      ...
      await expect(modal.locator('[data-testid="last-reminder-sent"]')).not.toContainText(
        'No reminder sent',
      )
    }
  })
  ...
  test('should show "No reminder sent" if no reminder has been sent', async ({ page }) => {
    ...
    const assignmentWithoutReminder = page
      .locator('[data-testid="assignment-row"][data-has-reminder="false"]')
      .first()
    ...
    await expect(modal.locator('[data-testid="last-reminder-sent"]')).toContainText(
      /No reminder sent/i,
    )
  })
  ...
  test('should display correct data for specific assignment', async ({ page }) => {
    ...
    const assigneeName = await assignmentRow
      .locator('[data-testid="row-assignee-name"]')
      .textContent()
...
test.describe('Assignment Details View - RTL (Arabic)', () => {
  ...
  test('should use logical properties for spacing (no ml-*, mr-*, pl-*, pr-*)', async ({
    page,
  }) => {
    // Open modal
    await page.locator('[data-testid="view-assignment-button"]').first().click()
