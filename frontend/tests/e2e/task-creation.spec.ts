 ...
 *
 * Prerequisites:
 * - User must be logged in ($TEST_USER_EMAIL)
...
test.describe('Task Creation - User Story 1', () => {
  ...
  test('T038: should create task with title and display it in task list', async ({ page }) => {
    ...
    const taskDescription =
      'Analyze and review the population data collection methodology for accuracy and compliance with international standards.'
    ...
    const taskListContent = await page
      .locator('[data-testid="task-list"], .task-list, main')
      .textContent()
  ...
  test('T039: should validate task title - empty and max length', async ({ page }) => {
    ...
    await expect(page.locator('text=/success|created|saved/i').first())
...
test.describe('Task List - Mobile & RTL Compliance', () => {
  ...
  test('should support RTL layout for Arabic language', async ({ page }) => {
    ...
    const langSelector = page.locator(
      '[data-testid="language-selector"], button:has-text("English"), select[name="language"]',
    )
