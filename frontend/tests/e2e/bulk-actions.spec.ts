...
test.describe('Bulk Reminder Actions - E2E (T042)', () => {
  ...
  test('should select 5 items and send bulk reminders', async ({ page }) => {
    ...
    await expect(page.locator('text=Successfully sent 5 reminders')).toBeVisible({ timeout: 60000 })
  ...
  test('should show progress updates during bulk processing', async ({ page }) => {
    ...
    while (Date.now() - startTime < 30000) {
      const progressText = await page.locator('[data-testid="bulk-progress-text"]').textContent()
  ...
  test('should allow selecting all items', async ({ page }) => {
    ...
    await expect(page.locator(`text=${totalCount} items selected`)).toBeVisible()
  ...
  test('should handle partial failures gracefully', async ({ page }) => {
    ...
    const summaryText = await page.locator('[data-testid="bulk-result-summary"]').textContent()
  ...
  test('should prevent bulk action exceeding 100 items', async ({ page }) => {
    ...
    if (count > 100) {
      ...
      await expect(page.locator('[data-testid="bulk-send-reminders-btn"]')).toBeDisabled()

      // Error message should be shown
      await expect(page.locator('text=Bulk actions limited to 100 items')).toBeVisible()
    }
  })
