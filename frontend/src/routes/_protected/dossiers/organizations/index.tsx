    ...
    >
      ...
      <header className="mb-6 flex flex-col gap-4 sm:mb-8 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-3">
          <Building2 className="size-6 text-primary sm:size-8" />
          <div>
            <h1 className="text-2xl font-bold sm:text-3xl md:text-4xl">{t('type.organization')}</h1>
      ...
      {!isLoading && !error && filteredDossiers?.length === 0 && (
        <div className="flex flex-col items-center justify-center py-12 text-center sm:py-16 lg:py-20">
          <Building2 className="mb-4 size-12 text-muted-foreground sm:size-16" />
                ...
                {filteredDossiers.map((dossier) => (
                  <TableRow key={dossier.id}>
                    <TableCell className="font-medium">
                      <Link
                        to={getDossierDetailPath(dossier.id, 'organization')}
                    ...
                    <TableCell className="text-end">
                      <Button variant="ghost" size="sm" asChild>
                        <Link to={getDossierDetailPath(dossier.id, 'organization')}>
                          {t('action.view')}
                        </Link>
                      </Button>
                    </TableCell>
