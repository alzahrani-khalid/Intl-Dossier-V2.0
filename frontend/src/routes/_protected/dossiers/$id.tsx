...
function DossierDetailPage() {
 ...
 {dossier.dossier_type === 'Organization' && <OrganizationTimeline dossierId={id} />}
 {dossier.dossier_type === 'Person' && <PersonTimeline dossierId={id} />}
 {dossier.dossier_type === 'Engagement' && <EngagementTimeline dossierId={id} />}
 {!['Country', 'Organization', 'Person', 'Engagement'].includes(dossier.dossier_type) && (
 <div className="text-center py-12 text-muted-foreground">
 <p>{t('timeline.not_supported', 'Timeline not yet supported for this dossier type')}</p>
 </div>
 )}
 </div>
 )}
