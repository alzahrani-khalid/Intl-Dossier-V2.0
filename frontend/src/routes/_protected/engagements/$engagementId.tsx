...
import { Calendar, MapPin, Users, FileText, ArrowLeft, LayoutGrid, Sparkles } from 'lucide-react'
...
function EngagementDetailPage() {
  ...
  const { t: tBrief } = useTranslation('ai-brief')
  ...
  // Handle when brief is generated - fetch the brief data
  const handleBriefGenerated = async (briefId: string) => {
    setBriefDialogOpen(false)
    // Fetch the brief data to display in viewer using Supabase session
    try {
      const {
        data: { session },
      } = await supabase.auth.getSession()
      if (!session?.access_token) {
        console.error('No active session for fetching brief')
        return
      }
  ...
  // Handle citation click to navigate to the source entity
  const handleCitationClick = (type: string, id: string) => {
    switch (type) {
      case 'dossier':
        ...
        break
      case 'position':
        // Positions are typically viewed within dossiers - no direct navigation
        break
      case 'commitment':
        // Navigate to commitments page with id search param
  ...
  return (
    <div className={`container mx-auto p-6 space-y-6 ${isRTL ? 'rtl' : 'ltr'}`}>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground">{t('engagements.detail')}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          ...
          >
            <Sparkles className={`h-4 w-4 ${isRTL ? 'rotate-180' : ''}`} />
            {tBrief('generate', 'Generate Brief')}
          </Button>
          <Button variant="outline" onClick={() => setKanbanOpen(true)} className="gap-2">
      ...
      <EngagementKanbanDialog
        ...
        columns={columns}
        stats={
          stats || { total: 0, todo: 0, in_progress: 0, review: 0, done: 0, progressPercentage: 0 }
        }
        ...
        <DialogContent className="sm:max-w-lg" dir={isRTL ? 'rtl' : 'ltr'}>
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              ...
              {tBrief('title', 'Generate AI Brief')}
            </DialogTitle>
          </DialogHeader>
      ...
      <Dialog open={briefViewerOpen} onOpenChange={setBriefViewerOpen}>
        ...
        >
          {generatedBrief && (
            <BriefViewer brief={generatedBrief} onCitationClick={handleCitationClick} />
          )}
        </DialogContent>
      </Dialog>
