...
export function useOptimisticLocking(options: UseOptimisticLockingOptions = {}) {
  ...
  const resolveConflictRef = useRef<((strategy: ConflictResolutionStrategy) => void) | null>(null)
   ...
   */
  const resolveConflict = useCallback((strategy: ConflictResolutionStrategy) => {
