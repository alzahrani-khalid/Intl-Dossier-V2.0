...
async function attachPosition(params: AttachPositionParams): Promise<AttachPositionResult> {
  ...
  if (error) {
    if (error.code === '23505') {
      ...
      throw new Error(
        'POSITION_ALREADY_ATTACHED: This position is already attached to the engagement',
      )
    }
...
export function useAttachPosition() {
  ...
  return useMutation<AttachPositionResult, Error, AttachPositionParams, OptimisticContext>({
