...
export function useAccessReviewSummary(reviewId: string | null) {
  ...
  return {
    ...
    inactiveUsers:
      reviewDetail?.findings?.filter((f: any) => f.issues?.includes('inactive_90_days')).length ||
      0,
    excessivePermissions:
      reviewDetail?.findings?.filter((f: any) => f.issues?.includes('excessive_permissions'))
        .length || 0,
    expiringGuests:
      reviewDetail?.findings?.filter((f: any) => f.issues?.includes('guest_expiring_soon'))
        .length || 0,
    certifiedUsers:
      reviewDetail?.findings?.filter((f: any) => f.certified_by && f.certified_at).length || 0,
  }
}
