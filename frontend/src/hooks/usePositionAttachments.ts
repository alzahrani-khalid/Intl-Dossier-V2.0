  ...
  return useMutation({
    mutationFn: async (file: File) => {
      ...
      if (!ALLOWED_MIME_TYPES.includes(file.type as any)) {
        throw new Error('File type not allowed. Allowed types: PDF, DOCX, XLSX, PNG, JPG')
      }
  ...
  return useMutation({
    ...
    onError: (_err, _attachmentId, context) => {
      ...
      if (context?.previousAttachments) {
        queryClient.setQueryData(['position-attachments', positionId], context.previousAttachments)
      }
    },
    onSuccess: () => {
