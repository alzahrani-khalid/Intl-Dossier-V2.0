...
async function fetchBriefingPackStatus(jobId: string): Promise<BriefingPackJobStatus> {
...
export function useBriefingPackStatus(options: UseBriefingPackStatusOptions) {
  const { jobId, enabled = true, onCompleted, onFailed } = options
