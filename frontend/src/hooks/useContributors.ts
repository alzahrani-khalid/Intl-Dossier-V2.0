...
export function useAddContributor(taskId: string) {
  ...
  return useMutation({
    mutationFn: (data: AddContributorRequest) => contributorsAPI.addContributor(taskId, data),
    ...
    onMutate: async (newContributor) => {
      ...
      if (previousContributors) {
        ...
        queryClient.setQueryData<TaskContributor[]>(contributorsKeys.task(taskId), [
          ...previousContributors,
          optimisticContributor,
        ])
      }
    ...
    onSuccess: (contributor) => {
      ...
      queryClient.setQueryData<TaskContributor[]>(contributorsKeys.task(taskId), (old) => {
