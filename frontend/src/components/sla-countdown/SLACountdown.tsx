...
export function SLACountdown({
  ...
  const getStatusColor = (): string => {
    if (isBreached) return 'bg-danger text-white'
    if (isPaused) return 'bg-muted-foreground text-white'

    const percentRemaining = (remainingMinutes / targetMinutes) * 100

    if (percentRemaining > 25) return 'bg-success text-white'
    if (percentRemaining > 10) return 'bg-warning text-ink'
      ...
      <div className="h-2 w-full overflow-hidden rounded-full bg-muted">
        <div
          className={`h-full transition-all duration-1000 ${
            isBreached ? 'bg-danger' : isPaused ? 'bg-muted-foreground' : 'bg-success'
          }`}
        ...
        {canPauseResume && !isBreached && (
          <button
            ...
            className={`rounded px-2 py-1 text-xs font-medium transition-colors disabled:opacity-50 ${
              isPaused
                ? 'bg-success/10 text-success hover:bg-success/20 dark:bg-success/30 dark:hover:bg-success/50'
                : 'bg-warning/10 text-warning hover:bg-warning/20 dark:bg-warning/30 dark:hover:bg-warning/50'
            }`}
