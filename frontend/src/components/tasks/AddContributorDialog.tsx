...
export function AddContributorDialog({
 ...
 {t(
 'tasks.addContributorDescription',
 ...
 <Search className={`absolute top-3 ${isRTL ? 'right-3' : 'left-3'} size-4 text-muted-foreground`} />
 <Input
 id="user-search"
 type="text"
 ...
 />
 {isSearching && (
 <Loader2 className={`absolute top-3 ${isRTL ? 'left-3' : 'right-3'} size-4 animate-spin`} />
 )}
 </div>
 ...
 <Select value={selectedRole} onValueChange={(value) => setSelectedRole(value as ContributorRole)}>
 ...
 {t(`tasks.contributorRole.${role}`, role)}
 </span>
 </SelectItem>
 ))}
 </SelectContent>
 ...
 {t('tasks.contributorNotes', 'Notes')} <span className="text-muted-foreground">({t('common.optional', 'Optional')})</span>
 </Label>
 <Textarea
 id="notes"
