...
function StatCard({
  ...
  const baseClasses =
    'flex items-center gap-3 rounded-lg bg-muted/50 p-3 transition-all duration-200'
    ...
    return (
      <div className="space-y-4">
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {[...Array(3)].map((_, i) => (
            <Skeleton key={`skeleton-top-${i}`} className="h-24 w-full" />
          ))}
        </div>
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          {[...Array(4)].map((_, i) => (
            <Skeleton key={`skeleton-bottom-${i}`} className="h-24 w-full" />
          ))}
        </div>
    ...
    <div className="space-y-4" dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        ...
        <StatCard
          ...
          iconColor={
            (stats.commitments?.overdue ?? 0) > 0 ? 'text-destructive' : 'text-muted-foreground'
          }
        />
      </div>
                ...
                {stats.health?.components && (
                  <div className="space-y-1 rounded-md bg-muted/50 p-2 text-xs">
                    ...
                    <div className="flex justify-between">
                      <span>
                        {t('stats.engagementFrequency', { ns: 'translation' }) ||
                          'Engagement Frequency'}
                      </span>
                      <span className="font-medium">
                        {stats.health.components.engagementFrequency}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span>
                        {t('stats.commitmentFulfillment', { ns: 'translation' }) ||
                          'Commitment Fulfillment'}
                      </span>
                      <span className="font-medium">
                        {stats.health.components.commitmentFulfillment}
                      </span>
                    </div>
                    <div className="flex justify-between">
