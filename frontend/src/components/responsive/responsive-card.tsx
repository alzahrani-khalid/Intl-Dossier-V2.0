  ...
  return (
    ...
    >
      {(title || description) && (
        ...
        >
          <div className="flex items-center justify-between">
            {title && (
              <CardTitle className={cn('text-base sm:text-lg md:text-xl', isMobile && 'text-sm')}>
                {title}
              </CardTitle>
            )}
          ...
          {description && !isCurrentlyCollapsed && (
            <CardDescription className={cn('text-xs sm:text-sm', isMobile && 'text-xs')}>
              {description}
            </CardDescription>
          )}
        </CardHeader>
      ...
      {footer && !isCurrentlyCollapsed && (
        <CardFooter className={cn('pt-4', isMobile && 'p-3 pt-2')}>{footer}</CardFooter>
      )}
    </Card>
  )
}
