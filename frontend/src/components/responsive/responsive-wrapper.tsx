...
export function ResponsiveWrapper<E extends ElementType = 'div'>(
  props: ResponsiveWrapperProps<E> &
    Omit<React.ComponentPropsWithoutRef<E>, keyof ResponsiveWrapperProps<E>>,
) {
  const {
    as,
    className,
    ...
    ...rest
  } = props as ResponsiveWrapperProps & Record<string, unknown>
  ...
  return (
    <Component
      ...
      style={
        enableContainer
          ? ({ containerType: 'inline-size', containerName: 'responsive' } as React.CSSProperties)
          : undefined
      }
      {...(rest as any)}
    >
 ...
 */
export function withResponsive<P extends object>(Component: React.ComponentType<P>) {
  type E = 'div'
  const Wrapped: React.FC<P & ResponsiveWrapperProps<E>> = ({
    className,
