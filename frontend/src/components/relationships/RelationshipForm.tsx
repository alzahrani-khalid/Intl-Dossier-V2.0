...
export function RelationshipForm({
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4" dir={isRTL ? 'rtl' : 'ltr'}>
        ...
        <div className="space-y-2">
          ...
          >
            <SelectTrigger id="to_contact_id" className="h-11 sm:h-10">
              <SelectValue
                placeholder={t('contactDirectory.relationships.form.select_contact_placeholder')}
              />
            </SelectTrigger>
            <SelectContent>
      ...
      <div className="space-y-2">
        ...
        >
          ...
          <SelectContent>
            ...
            <SelectItem value="other">{t('contactDirectory.relationshipTypes.other')}</SelectItem>
          </SelectContent>
        </Select>
      </div>
      ...
      {/* Start Date */}
      <div className="space-y-2">
        <Label htmlFor="start_date">{t('contactDirectory.relationships.form.start_date')}</Label>
        <Input id="start_date" type="date" {...register('start_date')} className="h-11 sm:h-10" />
      </div>

      {/* End Date */}
      <div className="space-y-2">
        <Label htmlFor="end_date">{t('contactDirectory.relationships.form.end_date')}</Label>
        <Input id="end_date" type="date" {...register('end_date')} className="h-11 sm:h-10" />
      </div>

      {/* Notes */}
      <div className="space-y-2">
        <Label htmlFor="notes">{t('contactDirectory.relationships.form.notes')}</Label>
        <Textarea
          id="notes"
          {...register('notes')}
