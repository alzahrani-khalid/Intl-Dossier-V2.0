  ...
  return (
    <div className="flex flex-wrap items-center gap-2 py-2">
      ...
      {filters.overdue && (
        <Badge
          variant="secondary"
          className="flex items-center gap-1 pe-1 text-xs bg-danger/10 dark:bg-danger/30 text-danger"
        >
          <span>{t('filters.overdue')}</span>
          <button
            type="button"
            onClick={() => onRemoveFilter('overdue')}
            className="ms-1 rounded-full hover:bg-danger/20 dark:hover:bg-danger/70 p-0.5 min-w-6 min-h-6 flex items-center justify-center"
