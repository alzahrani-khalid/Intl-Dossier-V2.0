...
export function EngagementPositionsSection({ engagementId }: EngagementPositionsSectionProps) {
  const { t } = useTranslation(['positions', 'common'])
  const [showAttachDialog, setShowAttachDialog] = useState(false)
  const [activeTab, setActiveTab] = useState<'attached' | 'suggestions' | 'briefing'>('attached')
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription>{t('positions:engagement_section.subtitle')}</CardDescription>
          </div>
          <Button onClick={() => setShowAttachDialog(true)} size="sm" className="gap-2">
      ...
      <CardContent>
        <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as any)}>
          <TabsList className="grid w-full grid-cols-3">
            ...
            <TabsTrigger value="briefing" disabled={attachedCount === 0} className="gap-2">
          ...
          <TabsContent value="attached" className="mt-6">
            {attachedError ? (
              ...
              <PositionList
                positions={attachedPositions.map(
                  (ep) => ep.positions as unknown as import('@/types/position').Position,
                )}
                isLoading={isLoadingAttached}
                context="engagement"
          ...
          <TabsContent value="suggestions" className="mt-6">
            {suggestionsError ? (
              ...
              <PositionSuggestionsPanel engagementId={engagementId} />
            )}
          </TabsContent>
          ...
          <TabsContent value="briefing" className="mt-6">
            {attachedCount === 0 ? (
              ...
              <BriefingPackGenerator
                ...
                attachedPositionCount={attachedCount}
              />
            )}
          </TabsContent>
        </Tabs>
