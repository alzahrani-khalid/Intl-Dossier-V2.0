...
export const PositionSuggestionsPanel: React.FC<PositionSuggestionsPanelProps> = ({
  ...
  const suggestions = (rawSuggestions as unknown as PositionSuggestion[]) || []
  const isFallbackMode = meta?.fallback_mode || false

  // Mutation for updating suggestion actions
  const updateActionMutation = useUpdateSuggestionAction()
        ...
        {suggestions.length === 0 ? (
          ...
          <div className="space-y-3" role="list" aria-label={t('positions.suggestions.listLabel')}>
            {suggestions.map((suggestion) => {
              ...
              const isActioned =
                suggestion.user_action === 'accepted' || suggestion.user_action === 'rejected'
                  ...
                  <CardContent className="p-4">
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <div className="flex-1 space-y-2">
                        ...
                        {/* Reasoning Keywords */}
                        {suggestion.suggestion_reasoning?.keywords &&
                          suggestion.suggestion_reasoning.keywords.length > 0 && (
