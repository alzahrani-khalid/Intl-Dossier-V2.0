  ...
  if (status === 'error') {
    return (
      <Alert variant="destructive" dir={isRTL ? 'rtl' : 'ltr'}>
        ...
        <AlertDescription className="text-sm">{error || t('ai.extraction.error')}</AlertDescription>
      </Alert>
    )
  }
              ...
              <div className="flex-1">
                ...
                <CardDescription className="text-xs sm:text-sm">
                  ...
                  {result.processing_time_ms && (
                    <span className="ms-2">({formatTime(result.processing_time_ms)})</span>
                  )}
                </CardDescription>
              </div>
            </div>
                    ...
                    <div key={idx} className="p-3 border rounded-lg bg-muted/50">
                      <div className="flex items-start justify-between gap-2 mb-2">
                        ...
                        <Badge variant={badge.variant} className="flex-shrink-0 text-xs">
                          {badge.icon && (
                            <badge.icon className={`h-3 w-3 ${isRTL ? 'ms-1' : 'me-1'}`} />
                          )}
                  ...
                  return (
                    <div key={idx} className="p-3 border rounded-lg bg-muted/50">
                      ...
                      <div className="flex items-center gap-4 text-xs text-muted-foreground">
                        <span>
                          {t('ai.extraction.owner')}: {commitment.owner_name}
                        </span>
                        <span>
                          {t('ai.extraction.due')}: {commitment.due_date}
                        </span>
                      </div>
                    </div>
                  )
