...
import { realtimeManager, type BroadcastPayload } from '../../lib/realtime'
...
export function RealtimeActivityFeed({
  ...
  const [selectedEntityTypes, setSelectedEntityTypes] = useState<Set<Activity['entityType']>>(
    new Set(),
  )
    ...
    return (
      <div className="flex-1 min-w-0">
        <div className="flex items-center space-x-2">
          ...
          <span className="font-medium text-sm text-gray-900">{activity.actorName}</span>
          <span className="text-sm text-gray-600">{activity.description}</span>
        </div>
        <div className="mt-1 flex items-center space-x-2">
          <EntityIcon className="h-3 w-3 text-gray-400" />
          <span className="text-sm font-medium text-gray-700">{activity.entityName}</span>
        </div>
        {activity.metadata?.comment && (
          <div className="mt-2 text-sm text-gray-600 italic">"{activity.metadata.comment}"</div>
        )}
    ...
    <div className={cn('bg-white rounded-lg shadow', className)}>
      ...
      <div ref={scrollContainerRef} className="max-h-96 overflow-y-auto">
        {isLoading ? (
          ...
          <div className="px-4 py-8 text-center text-gray-500">No activities to show</div>
        ) : (
  ...
  const trackActivity = useCallback(
    (
      ...
      realtimeManager.broadcast(
        'global-activity',
        'new_activity',
        activity as unknown as BroadcastPayload,
      )
