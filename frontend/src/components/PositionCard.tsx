...
export function PositionCard({ position }: PositionCardProps) {
  ...
  const handleClick = () => {
    navigate({ to: '/positions/$positionId', params: { positionId: position.id } as any })
  }
        ...
        <div className="space-y-3">
          ...
          {position.consistency_score !== undefined && position.consistency_score !== null && (
            <div className="flex items-center gap-2">
              <span className="text-xs text-muted-foreground">{t('fields.consistencyScore')}:</span>
              <Badge
                variant={
                  position.consistency_score >= 80
                    ? 'default'
                    : position.consistency_score >= 60
                      ? 'secondary'
                      : 'destructive'
                }
                className="text-xs"
          ...
          <div className="border-t pt-2 text-xs text-muted-foreground">
            <div className="flex items-center justify-between">
              ...
              <span>
                {t('fields.updated')}:{' '}
                {new Date(position.updated_at).toLocaleDateString(i18n.language)}
              </span>
            </div>
          </div>
        </div>
