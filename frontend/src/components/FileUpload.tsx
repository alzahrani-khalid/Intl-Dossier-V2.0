...
import { Upload, X, Pause, Play, RotateCcw, Check, AlertCircle, File, Trash2 } from 'lucide-react'
        ...
        try {
          const uploadId = await uploadFile(file, {
            ...
            onProgress: (_progress) => {
              // Progress update handled by uploadFile
            },
            onComplete: (url) => {
            ...
            {uploads.map((upload) => (
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  {upload.error && (
                    <p className="mt-1 truncate text-xs text-red-500">{upload.error}</p>
                  )}
                </div>

                <div className="flex items-center space-x-1">
                  {upload.status === 'uploading' && (
                    <Button size="sm" variant="ghost" onClick={() => pauseUpload(upload.id)}>
                      <Pause className="size-3" />
                    </Button>
                  )}

                  {upload.status === 'paused' && (
                    <Button size="sm" variant="ghost" onClick={() => resumeUpload(upload.id)}>
