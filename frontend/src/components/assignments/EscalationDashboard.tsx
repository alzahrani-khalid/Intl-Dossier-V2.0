  ...
  if (error) {
    return (
      <Alert variant="destructive">
        ...
        <AlertDescription>{t('escalation_dashboard.error_loading')}</AlertDescription>
      </Alert>
    )
  }
  ...
  return (
    <div className={cn('space-y-6', isRTL && 'rtl')}>
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">{t('escalation_dashboard.title')}</h1>
          <p className="text-muted-foreground">{t('escalation_dashboard.description')}</p>
        </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-lg font-medium">
              {isLoading
                ? '...'
                : t(`escalation_dashboard.reason.${report?.summary.most_common_reason}`)}
            </div>
          </CardContent>
        </Card>
                ...
                <div className="space-y-2">
                  {report?.time_series.map((item) => (
                    ...
                    >
                      ...
                      <div className="w-12 text-end font-semibold">{item.count}</div>
                    </div>
                  ))}
                </div>
              )}
        ...
        <TabsContent value="units" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>{t('escalation_dashboard.by_unit_description')}</CardDescription>
            </CardHeader>
            <CardContent>
                    ...
                    <div key={unit.unit_id} className="flex items-center gap-4">
                      <div className="flex-1">
                        ...
                        <div className="text-sm text-muted-foreground">
                          {unit.escalation_count} {t('escalation_dashboard.escalations')} (
        ...
        <TabsContent value="staff" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>{t('escalation_dashboard.by_staff_description')}</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-3">
                {isLoading ? (
                  ...
                  report?.by_assignee.slice(0, 10).map((assignee) => (
                    ...
                    >
                      <div>
                        ...
                        <div className="text-sm text-muted-foreground">
                          {assignee.escalation_count} / {assignee.total_assignments}{' '}
                          {t('escalation_dashboard.assignments')}
                        </div>
                      </div>
                      <div
                        className={cn(
                  ...
                  report?.by_work_type.map((workType) => (
                    <div key={workType.work_item_type} className="flex items-center gap-4">
                      <div className="flex-1">
                        ...
                        <div className="text-sm text-muted-foreground">
                          {workType.escalation_count} {t('escalation_dashboard.escalations')} (
