...
export const CommentForm = forwardRef<HTMLFormElement, CommentFormProps>(function CommentForm(
  { onSubmit, isSubmitting = false, maxLength = 5000, placeholder, users = [], onSearchUsers },
  ref,
) {
  ...
  const handleMentionSelect = (username: string): void => {
    ...
    if (lastAtSymbol !== -1) {
      const newText =
        textBeforeCursor.substring(0, lastAtSymbol + 1) + username + ' ' + textAfterCursor
      setText(newText)
      setShowMentions(false)
  ...
  return (
    <form ref={ref} onSubmit={handleSubmit} className="space-y-3" dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      {/* Keyboard Hint */}
      <p className="text-xs text-muted-foreground">{t('comments.form.keyboardHint')}</p>
    </form>
  )
})
