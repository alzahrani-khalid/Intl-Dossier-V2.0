  ...
  const { data: staffMembers, isLoading: loadingStaff } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from('staff_profiles')
        .select(
          'id, user_id, name_en, name_ar, current_assignment_count, individual_wip_limit, availability_status',
        )
      ...
      <DialogContent className="sm:max-w-[500px]">
        ...
        <div className="space-y-4 py-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="assignee">
              {t('assignments:manualOverride.selectAssignee')}{' '}
            ...
            {loadingStaff ? (
              ...
              <Select value={selectedAssigneeId} onValueChange={setSelectedAssigneeId}>
                <SelectTrigger id="assignee">
                  <SelectValue
                    placeholder={t('assignments:manualOverride.selectStaffPlaceholder')}
                  />
                </SelectTrigger>
                <SelectContent>
          ...
          <div className="space-y-2">
            <Label htmlFor="reason">
              {t('assignments:manualOverride.overrideReason')}{' '}
              <span className="text-red-500">*</span>
            </Label>
            <Textarea
              id="reason"
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400">
              {t('assignments:manualOverride.reasonHint')} ({overrideReason.length}/10{' '}
              {t('common:minChars')})
            </p>
            {validationError && (
        ...
        <DialogFooter>
          ...
          <Button
            ...
            disabled={
              manualOverrideMutation.isPending || !selectedAssigneeId || overrideReason.length < 10
            }
          >
