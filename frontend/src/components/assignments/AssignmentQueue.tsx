...
export function AssignmentQueue({ unitId, className }: AssignmentQueueProps) {
  ...
  >(undefined)
  const [workItemTypeFilter, setWorkItemTypeFilter] = useState<'ticket' | 'dossier' | undefined>(
    undefined,
  )
  ...
  const formatSkills = (
    skills: Array<{ skill_id: string; skill_name_en: string; skill_name_ar: string }>,
  ): string => {
    if (skills.length === 0) return t('common:none')
    const skillNames = skills.map((s) =>
      i18n.language === 'ar' ? s.skill_name_ar : s.skill_name_en,
    )
      ...
      {data && (
        <div className="flex gap-4 text-sm text-gray-600 dark:text-gray-400">
          ...
          <div>
            ...
            {(page - 1) * pageSize + 1}-{Math.min(page * pageSize, data.total_count)}{' '}
            {t('common:of')} {data.total_count}
          </div>
        </div>
      )}
              ...
              {data.items.length === 0 ? (
                ...
                data.items.map((item, index) => (
                  <TableRow key={item.queue_id}>
                    <TableCell className="font-mono">{(page - 1) * pageSize + index + 1}</TableCell>
                    <TableCell className="font-medium">{item.work_item_id}</TableCell>
                    <TableCell>
                      <Badge
                        className={cn(getWorkItemTypeColor(item.work_item_type), {
                          'me-auto': !isRTL,
                          'ms-auto': isRTL,
                        })}
                      >
                    ...
                    <TableCell>
                      <Badge
                        className={cn(getPriorityColor(item.priority), {
                          'me-auto': !isRTL,
                          'ms-auto': isRTL,
                        })}
                      >
