...
>(
  (
    ...
    // Keep a ref so setOpen can read the latest value without depending on `open`
    ...
    const setOpen = React.useCallback(
      (value: boolean | ((value: boolean) => boolean)) => {
        ...
        // Persist sidebar state via localStorage (SPA, no server cookies)
        try {
...
const SidebarRail = React.forwardRef<HTMLButtonElement, React.ComponentProps<'button'>>(
  ({ className, ...props }, ref) => {
    ...
    const actionLabel = state === 'collapsed' ? 'Expand Sidebar' : 'Collapse Sidebar'
    ...
    return (
      <button
        ...
        title={actionLabel}
        className={cn(
          'absolute inset-y-0 z-30 hidden w-6 -translate-x-1/2 items-center justify-center opacity-0 transition-all duration-200 ease-linear sm:flex',
          'group-data-[side=left]:-end-3 group-data-[side=right]:start-0 group-data-[side=right]:translate-x-1/2',
          'before:absolute before:inset-y-2 before:w-[2px] before:rounded-full before:bg-sidebar-border/70 before:transition-colors hover:before:bg-sidebar-ring',
        ...
        {...props}
      >
        <span className="relative z-10 flex size-5 items-center justify-center rounded-full border border-sidebar-border bg-background text-sidebar-foreground">
          <PanelLeft className="size-3.5 transition-transform duration-200 group-data-[state=collapsed]:rotate-180 rtl:-scale-x-100" />
        </span>
      </button>
    )
  },
  ...
  ({ className, ...props }, ref) => {
    return (
      <main
        ref={ref}
        className={cn(
          'relative flex min-w-0 flex-1 flex-col overflow-x-clip bg-background transition-[margin] duration-200 ease-linear',
...
>(({ className, asChild = false, ...props }, ref) => {
  ...
  return (
    <Comp
      ...
      className={cn(
        'flex h-8 shrink-0 items-center rounded-md px-2 text-xs font-medium text-sidebar-foreground/70 outline-none ring-sidebar-ring transition-[margin,opacity] duration-200 ease-linear focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0 font-text',
...
const sidebarMenuButtonVariants = cva(
  'peer/menu-button relative flex w-full items-center gap-2 overflow-hidden rounded-md p-2 text-start text-sm outline-none ring-sidebar-ring transition-[width,height,padding,color,background-color,box-shadow] hover:bg-sidebar-accent/90 hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 group-has-[[data-sidebar=menu-action]]/menu-item:pe-8 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[active=true]:bg-sidebar-primary/12 data-[active=true]:font-semibold data-[active=true]:text-sidebar-primary data-[active=true]:ring-1 data-[active=true]:ring-sidebar-primary/25 data-[state=open]:hover:bg-sidebar-accent data-[state=open]:hover:text-sidebar-accent-foreground group-data-[collapsible=icon]:!size-8 group-data-[collapsible=icon]:!p-2 [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0',
  {
    variants: {
      variant: {
