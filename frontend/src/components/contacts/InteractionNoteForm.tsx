          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 sm:space-y-6">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormDescription className="text-xs">
                    {field.value.length}/5000 {t('contactDirectory.interactions.form.characters')}
                  </FormDescription>
                  <FormMessage />
                </FormItem>
              )}
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                className="text-sm"
              />
              {attendeeSearch && contactsData && contactsData.data.length > 0 && (
                <div className="border rounded-md max-h-40 overflow-y-auto">
                  {contactsData.data.map((dossier) => (
                    <button
                      key={dossier.id}
                      type="button"
                      onClick={() => addAttendee(dossier.id)}
              ...
              {selectedAttendees.length > 0 && (
                <div className="flex flex-wrap gap-2">
                  {selectedAttendees.map((attendeeId) => {
                    const attendee = contactsData?.data.find((c) => c.id === attendeeId)
                    return (
                      <Badge key={attendeeId} variant="secondary" className="gap-1">
                        {attendee?.name_en || attendee?.name_ar || attendeeId}
                        <button
                          type="button"
