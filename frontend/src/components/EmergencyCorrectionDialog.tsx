...
export function EmergencyCorrectionDialog({
  ...
  const handleSubmit = async () => {
    ...
    if (!adminPassword.trim()) {
      setError(
        isRTL ? 'يرجى إدخال كلمة مرور المسؤول للتأكيد' : 'Please enter admin password to confirm',
      )
      return
    }
          ...
          <div className="rounded-lg bg-muted/50 p-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-muted-foreground">
                {isRTL ? 'سيظهر على الموقف المصحح' : 'Will appear on corrected position'}
              </span>
            </div>
          </div>
          ...
          {isAdmin && (
            <div className="space-y-2">
              ...
              <Input
                ...
                type="password"
                placeholder={
                  isRTL ? 'أدخل كلمة مرور المسؤول للتأكيد' : 'Enter your admin password to confirm'
                }
                value={adminPassword}
