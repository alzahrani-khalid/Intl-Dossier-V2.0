...
export const AuditLogViewer: React.FC<AuditLogViewerProps> = ({ ticketId, className = '' }) => {
  ...
  const getEventColor = (action: string): string => {
    ...
    if (action.includes('delete') || action.includes('close')) return 'text-red-600 bg-red-100'
  ...
  return (
    <div className={`space-y-4 ${className}`}>
      ...
      <div className="relative space-y-4">
        {/* Timeline line */}
        <div className={`absolute ${isRTL ? 'end-4' : 'start-4'} top-0 h-full w-0.5 bg-gray-200`} />
        ...
        {logs.map((log: AuditLogEntry) => {
          ...
          return (
            <div key={log.id} className={`relative ${isRTL ? 'pe-12 ps-0' : 'pe-0 ps-12'}`}>
              {/* Event icon */}
              <div
                className={`absolute ${
                  isRTL ? 'end-0' : 'start-0'
                  ...
                  <div className="mt-3 space-y-2 border-t pt-3">
                    {Object.entries(log.changes).map(
                      ([field, { old: oldValue, new: newValue }]) => (
                        <div key={field} className="rounded-md bg-gray-50 p-2 text-sm">
                          <div className="font-medium text-gray-700">
                            {t(`audit.fields.${field}`, field)}
                          </div>
                          <div className="mt-1 flex items-start gap-2">
                            <div className="flex-1">
                              <div className="text-xs text-gray-500">{t('audit.before')}:</div>
                              <div className="mt-0.5 rounded bg-red-50 px-2 py-1 text-red-800">
                                {JSON.stringify(oldValue)}
                              </div>
                            </div>
                            <div className="flex-1">
                              <div className="text-xs text-gray-500">{t('audit.after')}:</div>
