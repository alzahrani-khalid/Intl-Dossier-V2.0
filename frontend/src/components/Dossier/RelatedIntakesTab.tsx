...
import { intakeEntityLinksAPI as api } from '@/services/entity-links-api'
...
export function RelatedIntakesTab({ entityType, entityId }: RelatedIntakesTabProps) {
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: () =>
      api.getEntityIntakes(entityType as Parameters<typeof api.getEntityIntakes>[0], entityId, {
        page: currentPage,
        limit: PAGE_SIZE,
      }),
            ...
            <Card key={intake.id} className="hover:shadow-md transition-shadow">
              <CardHeader>
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                  <CardTitle className="text-base sm:text-lg">{intake.title}</CardTitle>
                  <Badge variant="outline" className="w-fit">
                    {t(`entity.link_type.${intake.link_type}`, intake.link_type)}
                  </Badge>
                </div>
                <CardDescription className="text-sm">
                  {new Date(intake.linked_at).toLocaleDateString(isRTL ? 'ar-SA' : 'en-US', {
                    year: 'numeric',
                    month: 'long',
                    day: 'numeric',
                  })}
                  {' • '}
                  {t('entity.related_intakes.linked_by', 'Linked by')}: {intake.linked_by}
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-2">
                <p className="text-sm text-muted-foreground">
                  {t('entity.related_intakes.ticket', 'Ticket')}: {intake.ticket_number}
                </p>
                <Button
                  variant="link"
                  size="sm"
