    ...
    <Card className="w-full" dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      <CardContent>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 sm:space-y-6">
            ...
            <Popover open={calendarOpen} onOpenChange={setCalendarOpen}>
              ...
              <PopoverContent className="w-auto p-0" align={isRTL ? 'end' : 'start'}>
                <Calendar
                  ...
                  disabled={(date) => date < minExpirationDate || date > maxExpirationDate}
                  initialFocus
                />
              </PopoverContent>
            </Popover>
          ...
          {/* Resource Selector */}
          <div className="space-y-2">
            <Label className="text-start">{t('user_management.allowed_resources')} *</Label>
            <p className="text-xs text-muted-foreground text-start">
            ...
            {selectedResources.length > 0 && (
              <div className="flex flex-wrap gap-2 p-3 border rounded-md bg-muted/50">
                {selectedResources.map((resourceId) => {
                  ...
                  return (
                    <Badge key={resourceId} variant="secondary" className="gap-1 ps-3 pe-2">
            ...
            <div className="border rounded-md p-4 max-h-64 overflow-y-auto space-y-3">
              {availableResources.length === 0 ? (
                <Alert>
                  ...
                  <AlertDescription>{t('user_management.no_resources_available')}</AlertDescription>
                </Alert>
              ) : (
