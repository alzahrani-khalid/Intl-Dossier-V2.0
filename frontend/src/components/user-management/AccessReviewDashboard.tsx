...
export function AccessReviewDashboard() {
  ...
  const [reviewScope, setReviewScope] = useState<'all_users' | 'department' | 'role' | 'custom'>(
    'all_users',
  )
  const [selectedReviewId, setSelectedReviewId] = useState<string | null>(null)

  const { mutate: generateReview, isPending: isGenerating } = useGenerateAccessReview()
  const { data: inProgressReviews, isLoading: loadingInProgress } =
    useAccessReviewsList('in_progress')
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-start">
            {t('user_management.access_reviews')}
          </h1>
        ...
        <Dialog open={createDialogOpen} onOpenChange={setCreateDialogOpen}>
          <DialogTrigger asChild>
            <Button className="w-full sm:w-auto ">{t('user_management.create_review')}</Button>
          </DialogTrigger>
          <DialogContent className="sm:max-w-md">
            <DialogHeader>
              <DialogTitle className="text-start">
                {t('user_management.create_access_review')}
              </DialogTitle>
            ...
            <div className="grid gap-4 py-4">
              ...
              <div className="grid gap-2">
                ...
                <Select
                  value={reviewScope}
                  onValueChange={(v) =>
                    setReviewScope(v as 'all_users' | 'department' | 'role' | 'custom')
                  }
                >
                  <SelectTrigger id="review-scope" className="">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                ...
                >
                  <CardHeader>
                    ...
                    <CardDescription className="text-start">
                      {t('user_management.reviewed_by')}:{' '}
                      {(review.reviewer as any)?.full_name || (review.reviewer as any)?.email}
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                    <div className="flex flex-col sm:flex-row sm:items-center gap-2 text-sm text-muted-foreground">
                      <span>
                        {t('user_management.started')}:{' '}
              ...
              {completedReviews?.map((review) => (
                <Card key={review.id}>
                  ...
                  <CardContent>
                    <div className="flex flex-col sm:flex-row sm:items-center gap-2 text-sm text-muted-foreground">
                      <span>
                        {t('user_management.completed_on')}:{' '}
