...
export function MfaSetupDialog({ open, onOpenChange, userId }: MfaSetupDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="max-w-[95vw] sm:max-w-md md:max-w-lg" dir={isRTL ? 'rtl' : 'ltr'}>
        <DialogHeader>
        ...
        {step === 'setup' && (
          <div className="flex flex-col gap-4 py-2 sm:py-4">
            ...
            <Button onClick={handleSetupMfa} disabled={setupMutation.isPending} className="w-full ">
        ...
        {step === 'verify' && setupData && (
          <div className="flex flex-col gap-4 py-2 sm:py-4">
            ...
            <div className="flex flex-col gap-2">
              <Label className="text-start text-sm">{t('userManagement.mfa.manualEntry')}</Label>
              <div className="flex gap-2">
                <Input value={setupData.secret} readOnly className="font-mono text-xs sm:text-sm" />
                <Button variant="outline" size="icon" onClick={handleCopySecret} className=" ">
