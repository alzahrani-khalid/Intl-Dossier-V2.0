...
/** UI-specific delegation shape (extends API Delegation with joined fields) */
interface DelegationDisplay {
  id: string
  grantor_id: string
  grantee_id: string
  expires_at: string
  status: string
  permissions?: string[]
  grantee?: { full_name?: string; username?: string }
  grantor?: { full_name?: string; username?: string }
}
...
export function DelegationManager({ userId, availablePermissions, users }: DelegationManagerProps) {
  ...
  const { createDelegation, revokeDelegation, validateDelegation, myDelegations } =
  ...
  const grantedDelegations = (myDelegations.data?.granted ?? []) as unknown as DelegationDisplay[]
  const receivedDelegations = (myDelegations.data?.received ?? []) as unknown as DelegationDisplay[]
  ...
  const onSubmit = async (values: DelegationFormValues) => {
    ...
    if (!validation.valid) {
      setValidationError(validation.issues?.[0]?.message || 'Delegation validation failed')
      return
    }
    ...
    await createDelegation.mutateAsync({
      ...
      valid_until: new Date(values.expiresAt).toISOString(),
    })
  ...
  const handleRevoke = async (delegationId: string) => {
    if (confirm(t('delegation.confirmRevoke'))) {
      await revokeDelegation.mutateAsync({ delegation_id: delegationId })
    }
  }
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
          <h2 className="text-2xl sm:text-3xl font-bold text-start">{t('delegation.title')}</h2>
            ...
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 sm:space-y-6">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel className="text-start block">{t('delegation.grantee')}</FormLabel>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel className="text-start block">{t('delegation.reason')}</FormLabel>
                      <FormControl>
                        <Textarea
                ...
                <DialogFooter className="flex flex-col-reverse sm:flex-row gap-2">
                  ...
                  >
                    {createDelegation.isPending ? t('delegation.creating') : t('delegation.create')}
                  </Button>
                </DialogFooter>
              </form>
            </Form>
                        ...
                        <TableRow key={delegation.id}>
                          <TableCell>
                            <div className="flex flex-col gap-1">
                              <span className="font-medium">{delegation.grantee?.full_name}</span>
                          ...
                          <TableCell className="hidden sm:table-cell">
                            <div className="flex flex-wrap gap-1">
                              ...
                              {(delegation.permissions?.length ?? 0) > 2 && (
                      ...
                      return (
                        <TableRow key={delegation.id}>
                          <TableCell>
                            <div className="flex items-center gap-2">
                              <div className="flex flex-col gap-1">
                                <span className="font-medium">{delegation.grantor?.full_name}</span>
                                <span className="text-xs text-muted-foreground">
