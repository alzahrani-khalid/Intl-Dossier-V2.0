...
export function EmptyAttention(): React.ReactElement {
  ...
  return (
    <div className="flex items-center gap-2 border border-success/50 bg-success/5 rounded-[var(--radius)] p-3 sm:p-4">
      <CircleCheck className="text-success shrink-0" size={20} />
      <span className="text-success font-semibold text-sm">{t('zones.attention.empty')}</span>
    </div>
  )
}
