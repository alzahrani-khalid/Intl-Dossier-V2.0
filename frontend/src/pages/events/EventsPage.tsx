...
function ListView({ events, isRTL, t }: ListViewProps) {
  const columns = [
    {
      key: 'title',
...
${event.type === 'meeting' ? 'bg-accent/10 text-accent' : ''}
${event.type === 'conference' ? 'bg-secondary/30 text-secondary-foreground' : ''}
${event.type === 'workshop' ? 'bg-success/10 text-success' : ''}
${event.type === 'training' ? 'bg-warning/10 text-warning' : ''}
${event.type === 'ceremony' ? 'bg-secondary/20 text-secondary-foreground' : ''}
${event.type === 'other' ? 'bg-muted text-ink-mute' : ''}
`}
        >
    ...
    },
    {
      ...
      cell: (event: Event) => (
        <span
          className={`
inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
${event.status === 'draft' ? 'bg-muted text-ink-mute' : ''}
${event.status === 'scheduled' ? 'bg-accent/10 text-accent' : ''}
${event.status === 'ongoing' ? 'bg-success/10 text-success' : ''}
${event.status === 'completed' ? 'bg-muted text-ink-mute' : ''}
${event.status === 'cancelled' ? 'bg-danger/10 text-danger' : ''}
`}
        >
