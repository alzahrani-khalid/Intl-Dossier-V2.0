  ...
  if (isLoadingUser) {
    return (
      <div
        ...
        dir={isRTL ? 'rtl' : 'ltr'}
      >
        <div className="text-center py-12 text-muted-foreground">{t('common:common.loading')}</div>
      </div>
    )
  }
  ...
  if (!user) {
    return (
      <div
        ...
        dir={isRTL ? 'rtl' : 'ltr'}
      >
        <div className="text-center py-12 text-muted-foreground">{t('errors.userNotFound')}</div>
      </div>
    )
  }
            ...
            <div className="flex flex-col sm:flex-row gap-4 sm:gap-6">
              ...
              <div className="flex-1">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  <div
                    className={`flex flex-row items-center gap-2 ${isRTL ? 'flex-row-reverse' : ''}`}
                  >
                  ...
                  {user.expires_at && (
                    ...
                    >
                      ...
                      <span className="text-sm text-orange-600">
                        ...
                        {new Date(user.expires_at).toLocaleDateString(isRTL ? 'ar' : 'en')}
                      </span>
                    </div>
                  )}
                </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription className="text-start">{t('roles.' + user.role)}</CardDescription>
              </CardHeader>
              <CardContent>
                    ...
                    {permissions?.effective_permissions ? (
                      <div className="border rounded-lg p-3">
                        <div className="font-medium text-sm mb-1">
                          {t('userDetail.effectivePermissions')}
                        </div>
                        <div className="flex flex-wrap gap-1">
                          {permissions.effective_permissions.accessible_resources.map(
                            (resource) => (
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription className="text-start">{t('delegation.received')}</CardDescription>
              </CardHeader>
              <CardContent>
                {isLoadingPermissions ? (
                  <div className="text-center py-4 text-muted-foreground">
                        ...
                        <div key={delegation.id} className="border rounded-lg p-3">
                          ...
                          {delegation.valid_until && (
                            <div className="text-xs text-muted-foreground">
                              {t('userDetail.validUntil', {
                                until: new Date(delegation.valid_until).toLocaleDateString(
                                  isRTL ? 'ar' : 'en',
                                ),
                              })}
                            </div>
              ...
              <div className="space-y-4">
                {user.last_login_at && (
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between p-3 border rounded-lg">
                    ...
                    {user.last_login_ip && <Badge variant="outline">{user.last_login_ip}</Badge>}
                  </div>
                )}
                {!user.last_login_at && (
