...
import type { ReviewFinding } from '@/services/user-management-api'
...
export function AccessReviewPage() {
  ...
  const { data: reviewDetail, isLoading } = useAccessReviewDetail(
    reviewId || null,
    findingTypeFilter === 'all' ? undefined : findingTypeFilter,
  )
    ...
    certifyAccess(
      {
        ...
        certified: approved,
        requested_changes: approved
          ? undefined
          : [
              {
                change_type:
                  (recommendedAction as
                    | 'reduce_role'
                    | 'remove_delegation'
                    | 'deactivate'
                    | 'other') || 'other',
  ...
  const getIssueBadgeVariant = (issue: string): 'destructive' | 'outline' | 'secondary' => {
    if (issue.includes('inactive')) return 'destructive'
    if (issue.includes('excessive')) return 'outline'
    return 'secondary'
  }
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-sm sm:text-base text-muted-foreground text-start mt-1">
              ...
              {t(`user_management.scope_${reviewDetail?.review_scope}`)}
            </p>
          </div>
        ...
        <div className="w-full sm:w-64">
          ...
          <Select value={findingTypeFilter} onValueChange={setFindingTypeFilter}>
            ...
            <SelectContent>
              ...
              <SelectItem value="inactive_90_days">
                {t('user_management.inactive_users')}
              </SelectItem>
              <SelectItem value="excessive_permissions">
                {t('user_management.excessive_permissions')}
              </SelectItem>
              <SelectItem value="guest_expiring_soon">
                {t('user_management.expiring_guests')}
              </SelectItem>
            </SelectContent>
          </Select>
        </div>
            ...
            <TableBody>
              {reviewDetail?.findings?.map((finding: Finding) => (
                <TableRow key={finding.user_id}>
                  ...
                  <TableCell>
                    <Badge variant="outline">{finding.primary_role}</Badge>
                  </TableCell>
                  <TableCell>
        ...
        {reviewDetail?.findings?.map((finding: Finding) => (
          <Card key={finding.user_id}>
            ...
            <CardContent className="space-y-3">
              ...
              <div>
                <Label className="text-xs text-muted-foreground">
                  {t('user_management.recommendations')}
                </Label>
        ...
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="text-start">
              {t('user_management.certify_user_access')}
            </DialogTitle>
          ...
          <Tabs defaultValue="approve" className="w-full">
            <TabsList className="grid w-full grid-cols-2">
              ...
              <TabsTrigger value="request_change">
                {t('user_management.request_change')}
              </TabsTrigger>
            </TabsList>
            ...
            <TabsContent value="request_change" className="mt-4 space-y-4">
              <div className="space-y-2">
                <Label htmlFor="recommended-action">
                  {t('user_management.recommended_action')}
                </Label>
                <Select value={recommendedAction} onValueChange={setRecommendedAction}>
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="revoke_delegations">
                      {t('user_management.revoke_delegations')}
                    </SelectItem>
