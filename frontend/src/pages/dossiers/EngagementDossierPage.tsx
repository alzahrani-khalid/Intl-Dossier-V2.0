  ...
  const headerActions = (
    <>
      ...
      >
        ...
        <span className="hidden sm:inline">{t('engagement.viewKanban')}</span>
      </Button>
      <Button asChild className="gap-2 min-h-11 min-w-11">
        <Link
          to="/engagements/$engagementId/after-action"
          params={{ engagementId: dossier.id } as any}
        >
          <FileText className="h-4 w-4" />
          <span className="hidden sm:inline">{t('engagement.logAfterAction')}</span>
        </Link>
      </Button>
    </>
  )
