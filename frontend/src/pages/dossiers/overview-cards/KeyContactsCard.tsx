 ...
 * KeyContactsCard
 *
 * Country-specific card showing up to 5 key contacts linked to this dossier.
 * Each contact shows name (bilingual), title/role, organization.
 * Clickable items navigate to person detail page.
...
export function KeyContactsCard({ dossierId }: KeyContactsCardProps): React.ReactElement {
  ...
  const displayContacts = contacts.slice(0, MAX_CONTACTS)
  const hasMore = totalCount > MAX_CONTACTS
