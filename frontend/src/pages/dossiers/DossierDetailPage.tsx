...
export function DossierDetailPage() {
  ...
  const { data: relationshipsData, isLoading: isLoadingRelationships } = useRelationshipsForDossier(id as string);
  ...
  const handleLinkDocument = async (documentId: string, documentType: 'position' | 'mou' | 'brief') => {
    // TODO: Implement document linking via API
    toast.success(t('document.linkSuccess'));
    setIsDocumentDialogOpen(false);
  };

  const handleRemoveDocument = async (documentId: string, documentType: 'position' | 'mou' | 'brief') => {
    ...
    return (
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6 lg:py-8" dir={isRTL ? 'rtl' : 'ltr'}>
        <Alert variant="destructive">
          ...
          <AlertDescription>
            {error?.message || t('detail.errorMessage')}
          </AlertDescription>
        </Alert>
        <Button onClick={handleBack} variant="outline" className="mt-4 min-h-11">
  ...
  return (
    <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6 lg:py-8" dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mb-6">
        <Button onClick={handleBack} variant="ghost" size="sm" className="min-h-11 self-start">
          <ArrowLeft className={cn('h-4 w-4', isRTL ? 'ms-2 rotate-180' : 'me-2')} />
          {t('detail.back')}
        </Button>
        <div className="flex flex-col-reverse sm:flex-row gap-2">
          <Button onClick={handleEdit} variant="outline" size="lg" className="min-h-11 w-full sm:w-auto">
            <Edit className={cn('h-4 w-4', isRTL ? 'ms-2' : 'me-2')} />
            {t('detail.edit')}
          </Button>
          <AlertDialog>
            <AlertDialogTrigger asChild>
              <Button variant="destructive" size="lg" className="min-h-11 w-full sm:w-auto">
      ...
      <Card>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} defaultValue="details" className="p-4 sm:p-6">
          ...
          <TabsContent value="relationships" className="space-y-4 mt-4 sm:mt-6">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
              ...
              <Button onClick={handleAddRelationship} className="min-h-11 w-full sm:w-auto">
                <Plus className={cn('h-4 w-4', isRTL ? 'ms-2' : 'me-2')} />
          ...
          <TabsContent value="calendar">
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <Button variant="outline" size="sm" onClick={() => navigate({ to: '/calendar' })} className="min-h-9">
