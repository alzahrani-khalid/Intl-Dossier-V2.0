...
import { Upload, FileText, Image, Video, Database, Download, Trash2, Tag } from 'lucide-react'
 ...
 `,
  ...
  const uploadMutation = useMutation({
    mutationFn: async (file: File) => {
      ...
      const {
        data: { publicUrl },
      } = supabase.storage.from('data-library').getPublicUrl(fileName)

      // Create database record
      const { error: dbError } = await supabase.from('data_library_items').insert({
