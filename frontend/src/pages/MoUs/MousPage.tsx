    ...
    () => [
      {
        ...
        cell: ({ row }) => <div className="font-mono text-sm">{row.original.reference_number}</div>,
      },
      {
        id: 'title',
        header: t('mous.title'),
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <div className="text-sm">
            <div>
              {isRTL ? row.original.primary_party.name_ar : row.original.primary_party.name_en}
            </div>
        ...
        cell: ({ row }) => {
          if (row.original.expiry_date) {
            const daysUntilExpiry = Math.floor(
              (new Date(row.original.expiry_date).getTime() - new Date().getTime()) /
                (1000 * 60 * 60 * 24),
            )
