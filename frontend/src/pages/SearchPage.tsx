...
export function SearchPage() {
  ...
  const searchParams = useSearch({ from: '/_protected/search' })
  ...
  function handleSearchSubmit() {
    ...
    if (useSemanticMode) {
      performSemanticSearch({
        query,
        entityTypes: selectedType === 'all' ? ['positions', 'documents'] : [selectedType],
  ...
  const hasNoResults =
    ...
    !displayData?.exactMatches?.length
  ...
  return (
    <SearchErrorBoundary>
      <div className="container mx-auto max-w-6xl px-4 py-6" dir={isRTL ? 'rtl' : 'ltr'}>
        ...
        {/* Search Results */}
        {searchError ? (
          <SearchEmptyState type="error" onRetry={() => refetchSearch()} />
        ) : hasNoResults ? (
          <SearchEmptyState
            type="no-results"
            searchQuery={query}
