...
export function AfterActionEditPage() {
  ...
  const { mutateAsync: publishAfterAction, isPending: _isPublishing } = usePublishAfterAction()
  ...
  const handleSaveDraft = async (data: AfterActionFormData, _isDraft: boolean) => {
    try {
      ...
      // Cast form data to update input - the shapes are compatible at runtime
      const updatePayload = {
        ...data,
        _version: afterAction?._version || 1,
      } as unknown as { [key: string]: unknown }
      await (updateAfterAction as (vars: { id: string; data: unknown }) => Promise<unknown>)({
        id,
        data: updatePayload,
      })
  ...
  const handlePublish = async (data: AfterActionFormData) => {
    try {
      ...
      // First save any pending changes - cast form data to update input
      const updatePayload = {
        ...data,
        _version: afterAction?._version || 1,
      } as unknown as { [key: string]: unknown }
    ...
    return (
      ...
      >
        <Alert variant="destructive">
          ...
          <AlertDescription>{t('afterActions.edit.notFound')}</AlertDescription>
        </Alert>
      </div>
    )
  }
  ...
  return (
    ...
    >
      ...
      <AfterActionForm
        ...
        initialData={afterAction as unknown as Partial<AfterActionFormData>}
