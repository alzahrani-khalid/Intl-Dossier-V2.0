...
import type { AfterActionRecord } from '../../../../backend/src/types/after-action.types'

/** Linked task returned by the API when fetching full after-action detail */
...
/** Extension of AfterActionRecord with linked_tasks that the API may include */
interface AfterActionRecordWithLinkedTasks extends AfterActionRecord {
...
export function AfterActionViewPage() {
  ...
  const { data: afterAction, isLoading, error } = useAfterActionDetail(id)
  ...
  if (isLoading) {
    return (
      <div
        className={cn('container mx-auto px-4 sm:px-6 lg:px-8 py-6 max-w-6xl', isRTL && 'rtl')}
        dir={isRTL ? 'rtl' : 'ltr'}
      >
    ...
    return (
      ...
      >
        <Alert variant="destructive">
          ...
          <AlertDescription>{t('afterActions.view.notFound')}</AlertDescription>
        </Alert>
      </div>
    )
  }
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        ...
        <div className="flex gap-2">
          {afterAction.status === 'draft' && (
            <Button variant="default" onClick={() => navigate({ to: `/after-actions/${id}/edit` })}>
        ...
        <CardContent className="space-y-2 text-sm">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              {afterAction.created_by}
            </div>
            <div>
              <span className="font-semibold">{t('afterActions.view.createdAt')}:</span>{' '}
              {format(new Date(afterAction.created_at!), 'PPp')}
            </div>
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1">
                      ...
                      {risk.mitigation_strategy && (
                        <p className="text-sm text-muted-foreground mt-2 text-start">
                          <strong>{t('afterActions.view.mitigation')}:</strong>{' '}
                          {risk.mitigation_strategy}
                        </p>
                      )}
      ...
      {/* Linked Tasks */}
      {(afterAction as AfterActionRecordWithLinkedTasks).linked_tasks &&
        (afterAction as AfterActionRecordWithLinkedTasks).linked_tasks!.length > 0 && (
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                <ListTodo className="h-5 w-5" />
                {t('afterActions.view.linkedTasks')} (
                {(afterAction as AfterActionRecordWithLinkedTasks).linked_tasks!.length})
              </CardTitle>
            </CardHeader>
            <CardContent>
              <div className="space-y-2">
                {(afterAction as AfterActionRecordWithLinkedTasks).linked_tasks!.map(
                  (task: LinkedTask) => (
                    <div
                      key={task.id}
