...
import { AlertCircle, Loader2, Plus, Search, FileText, Calendar, User, Filter } from 'lucide-react'
...
export function AfterActionListPage() {
  ...
  const {
    ...
    status:
      statusFilter !== 'all' ? [statusFilter as 'draft' | 'published' | 'edit_pending'] : undefined,
    search: searchQuery || undefined,
    page,
    limit: pageSize,
  })

  const afterActions = afterActionsData?.items || []
  const totalCount = afterActionsData?.pagination?.total_items || 0
  ...
  const handleCreateNew = () => {
    navigate({
      ...
      search: { dossier_id: dossierId } as any,
    })
  }
    ...
    return (
      ...
      >
        <Alert variant="destructive">
          ...
          <AlertDescription>{t('afterActions.list.errorLoading')}</AlertDescription>
        </Alert>
      </div>
    )
  }
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        ...
        {/* Create Button */}
        {dossierId && (
          <Button variant="default" onClick={handleCreateNew} className=" px-4 sm:px-6">
            ...
            <div className="w-full sm:w-48">
              ...
              >
                ...
                <SelectContent>
                  ...
                  <SelectItem value="edit_pending">
                    {t('afterActions.status.edit_pending')}
                  </SelectItem>
                </SelectContent>
              </Select>
            </div>
                ...
                <div className="flex flex-wrap gap-4 text-sm text-muted-foreground">
                  ...
                  {afterAction.decisions_count > 0 && (
                    <Badge variant="outline">
                      {t('afterActions.list.decisionsCount', {
                        count: afterAction.decisions_count,
                      })}
                    </Badge>
                  )}
                  {afterAction.commitments_count > 0 && (
                    <Badge variant="outline">
                      {t('afterActions.list.commitmentsCount', {
                        count: afterAction.commitments_count,
                      })}
                    </Badge>
                  )}
