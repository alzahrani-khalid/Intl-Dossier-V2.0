...
import type { WorkingGroupExtension } from '@/services/dossier-api'

/** Extended working group fields stored in extension JSON beyond the typed WorkingGroupExtension */
interface WorkingGroupExtensionWithExtra extends WorkingGroupExtension {
  chair_name?: string
  member_count?: number
...
export default function WorkingGroups() {
  ...
  const totalMembers = filteredGroups.reduce((acc, curr) => {
    const members =
      (curr.extension as WorkingGroupExtensionWithExtra | undefined)?.member_count || 0
    return acc + members
  }, 0)
  const totalMeetings = filteredGroups.reduce((acc, curr) => {
    const meetings =
      (curr.extension as WorkingGroupExtensionWithExtra | undefined)?.meetings_count || 0
    return acc + meetings
  }, 0)
        ...
        <table className="min-w-full divide-y divide-border text-sm">
          <thead>
            <tr className="bg-muted/50 text-start text-xs font-semibold uppercase tracking-wide text-muted-foreground">
              ...
              <th className="px-5 py-3 text-start">
                {t('workingGroups.table.members', 'Members')}
              </th>
          ...
          <tbody className="divide-y divide-border">
            {filteredGroups.map((dossier) => {
              const group = dossier.extension as WorkingGroupExtensionWithExtra | undefined
              return (
                <tr key={dossier.id} className="hover:bg-accent/50">
                  ...
                  <td className="px-5 py-4 text-foreground">
                    {(group as WorkingGroupExtensionWithExtra | undefined)?.meetings_count || 0}
                  </td>
