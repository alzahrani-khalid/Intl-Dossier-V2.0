...
async function fetchWithAuth<T>(endpoint: string, options: RequestInit = {}): Promise<T> {
  ...
  if (!data.success) {
    throw new AfterActionAPIError((data as APIErrorResponse).error)
  }
...
export const afterActionAPI = {
   ...
   */
  async update(id: string, data: AfterActionUpdateInput): Promise<AfterActionRecord> {
   ...
   */
  async publish(id: string, version: number, sendNotifications = true): Promise<PublishResponse> {
  ...
  async list(filters: AfterActionListFilters): Promise<AfterActionListResponse> {
    ...
    return fetchWithAuth<AfterActionListResponse>(`/after-action/list?${params.toString()}`, {
      method: 'GET',
    })
  },
  ...
  async get(
    ...
    return fetchWithAuth<AfterActionRecord>(`/after-action/get/${id}?${params.toString()}`, {
      method: 'GET',
    })
  },
