 ...
 */
export async function exportContacts(format: ExportFormat, filters?: ExportFilters): Promise<Blob> {
  try {
    ...
    const contentType = format === 'csv' ? 'text/csv;charset=utf-8' : 'text/vcard;charset=utf-8'
    ...
    throw new ExportApiError(500, error instanceof Error ? error.message : 'Export failed')
  }
}
