...
async function fetchWithAuth<T>(endpoint: string, options: RequestInit = {}): Promise<T> {
  ...
  if (!data.success) {
    throw new EntityLinksAPIError((data as APIErrorResponse).error)
  }
...
export const intakeEntityLinksAPI = {
   ...
   */
  async getLinks(intakeId: string, includeDeleted = false): Promise<EntityLink[]> {
    const params = new URLSearchParams()
    params.append('intake_id', intakeId)
    params.append('include_deleted', includeDeleted.toString())

    return fetchWithAuth<EntityLink[]>(`/intake-links-get?${params.toString()}`, {
      method: 'GET',
    })
  },
   ...
   */
  async updateLink(intakeId: string, linkId: string, data: UpdateLinkRequest): Promise<EntityLink> {
  ...
  async restoreLink(intakeId: string, linkId: string): Promise<EntityLink> {
    return fetchWithAuth<EntityLink>(`/intake/${intakeId}/links/${linkId}/restore`, {
      method: 'POST',
    })
  },
  ...
  async createBatchLinks(
    ...
    return fetchWithAuth<BatchCreateResponse>(`/intake-links-batch?intake_id=${intakeId}`, {
      method: 'POST',
      body: JSON.stringify(data),
    })
  },
   ...
   */
  async searchEntities(filters: EntitySearchFilters): Promise<EntitySearchResult[]> {
    const params = new URLSearchParams()
    ...
    if (filters.classification_level !== undefined) {
      params.append('classification_level', filters.classification_level.toString())
    }
    ...
    return fetchWithAuth<EntitySearchResult[]>(`/entities-search?${params.toString()}`, {
      method: 'GET',
    })
  },
  ...
  async getAuditLog(intakeId: string, linkId: string): Promise<LinkAuditLog[]> {
    return fetchWithAuth<LinkAuditLog[]>(`/intake/${intakeId}/links/${linkId}/audit-log`, {
      method: 'GET',
    })
  },
  ...
  ai: {
     ...
     */
    async acceptSuggestion(intakeId: string, data: AcceptSuggestionRequest): Promise<EntityLink> {
      return fetchWithAuth<EntityLink>(`/intake/${intakeId}/links/suggestions/accept`, {
        method: 'POST',
        body: JSON.stringify(data),
      })
    },
     ...
     */
    async rejectSuggestion(intakeId: string, suggestionId: string): Promise<void> {
      await fetchWithAuth<void>(`/intake/${intakeId}/links/suggestions/${suggestionId}/reject`, {
        method: 'POST',
      })
    },
  },
