...
export async function uploadBusinessCard(
  ...
  if (!validTypes.includes(file.type.toLowerCase())) {
    throw new OCRAPIError('Invalid file type. Please upload JPEG, PNG, or HEIC image.', 400)
  }

  // Validate file size (max 10MB)
  const maxSize = 10 * 1024 * 1024 // 10MB
  if (file.size > maxSize) {
    throw new OCRAPIError('File too large. Maximum size is 10MB.', 400)
  }
...
export function normalizeOCRFields(fields: OCRParsedFields): OCRParsedFields {
  ...
  if (normalized.email_addresses && normalized.email_addresses.length > 0) {
    normalized.email_addresses = Array.from(
      new Set(normalized.email_addresses.map((e) => e.toLowerCase().trim()).filter(isValidEmail)),
    )
  }
  ...
  if (normalized.phone_numbers && normalized.phone_numbers.length > 0) {
    normalized.phone_numbers = Array.from(
      new Set(normalized.phone_numbers.map((p) => p.trim()).filter(isValidPhone)),
    )
  }
...
export async function uploadDocument(file: File): Promise<DocumentUploadResponse> {
  ...
  if (file.size > maxSize) {
    throw new OCRAPIError('File too large. Maximum size is 50MB.', 400)
  }
  ...
  const response = await fetch(`${supabaseUrl}/functions/v1/contacts-extract-document`, {
    method: 'POST',
    headers: {
      Authorization: headers.Authorization as string, // Only send auth header, let browser set Content-Type with boundary
    },
    body: formData,
  })
