 ...
 * @module domains/search/hooks/useAdvancedSearch
 *
 * TanStack Query hook for complex multi-criteria search.
 * API calls delegated to search.repository.
 */

import { useMutation, useQuery, useQueryClient } from '@tanstack/react-query'
import { advancedSearch } from '../repositories/search.repository'
import type {
...
): ReturnType<typeof useQuery<AdvancedSearchResponse | null>> {
  return useQuery({
    queryKey: request ? advancedSearchKeys.search(request) : ['advanced-search', 'disabled'],
    queryFn: () => (request ? advancedSearch(request) : Promise.resolve(null)),
...
export function useAdvancedSearchMutation(): ReturnType<typeof useMutation<AdvancedSearchResponse, Error, AdvancedSearchRequest>> {
  const queryClient = useQueryClient()

  return useMutation({
    mutationFn: advancedSearch,
...
function useSearchHistory(): ReturnType<typeof useQuery<SearchHistoryItem[]>> {
  return useQuery({
...
export function buildSearchRequest(state: SearchState): AdvancedSearchRequest {
  ...
  if (state.query.trim()) request.query = state.query.trim()
  if (state.conditions.length > 0) request.conditions = state.conditions
  if (state.conditionGroups.length > 0) request.condition_groups = state.conditionGroups
  if (state.relationships.length > 0) request.relationships = state.relationships
  if (state.dateRange && (state.dateRange.from || state.dateRange.to || state.dateRange.preset)) {
    request.date_range = state.dateRange
  }
  if (state.status.length > 0) request.status = state.status
  if (state.tags.length > 0) request.tags = state.tags
  if (state.savedFilterId) request.saved_filter_id = state.savedFilterId
...
export { useAdvancedSearch, useSearchHistory, clearSearchHistory }
