...
export function DesignComplianceProvider({ children }: { children: React.ReactNode }) {
  ...
  const loadRules = useCallback(async (componentName?: string) => {
    ...
    const data = await safeFetch<{ rules: ValidationRule[] }>(
      `/api/validation/rules?${qs.toString()}`,
    )
    rulesRef.current = data?.rules
    return rulesRef.current ?? []
  }, [])

  const validate = useCallback<DesignComplianceContextValue['validate']>(
    async (input) => {
      ...
      const api: { passed: boolean; duration?: number; results: ValidationResult[] } | undefined =
        await safeFetch<{ passed: boolean; duration?: number; results: ValidationResult[] }>(
...
export function useDesignComplianceContext() {
  ...
  if (!ctx)
    throw new Error('useDesignComplianceContext must be used within DesignComplianceProvider')
  return ctx
}
