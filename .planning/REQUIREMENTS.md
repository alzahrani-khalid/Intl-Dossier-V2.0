...
- [ ] **BUG-05**: KPI labels render on a single line at 1024px (e.g. "ACTIVE ENGAGEMENTS" no longer wraps) via shortened/abbreviated label or reduced label size at ≤1024. _(F22, LOW)_
...
- [ ] **FMT-01**: `lib/format-date.ts` is the single date/time formatter, emitting day-first no-comma dates (`Tue 28 Apr`) and GST times (`14:30 GST`); the dashboard greeting and Intelligence Digest read day-first no-comma. _(F4, HIGH)_
...
- [ ] **FMT-04**: Arabic overdue units no longer mix scripts — kanban cards show a localized unit (`يوم`/`ي`) after Latin digits (no bare Latin `d`), applied via `lib/format-locale` `toFormatLocale` under the locked Latin-digit policy. _(F5, MEDIUM)_
