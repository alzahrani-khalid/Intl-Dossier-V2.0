...
| Option                                                      | Description                                                     | Selected |
| ----------------------------------------------------------- | --------------------------------------------------------------- | -------- |
| Both: window global + <FullscreenLoader> prop (Recommended) | window.\_\_showGlobeLoader(ms) (dev-gated) + React prop API.    | ✓        |
| React-only via useGlobeLoader() hook                        | useGlobeLoader() hook returning {show, hide}. No window global. |          |
| window global always (dev + prod)                           | Match handoff exactly — window.\_\_showGlobeLoader in prod.     |          |
