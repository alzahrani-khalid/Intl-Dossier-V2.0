...
export const test = base.extend<Fixtures>({
  dossierList: async ({ page }, use) => {
    await use(new DossierListPage(page))
  },
  cleanup: async ({}, use) => {
...
await page
  .locator('[data-testid="work-item-abc"]')
...
_Not applicable — Phase 18 adds new test files and CI config; no rename/refactor/migration._
...
| GitHub Actions secrets (E2E\_\*)         | CI workflow       | ✗ Must be added              | —       | Planner adds a task to create secrets              |
...
- [ ] `.env.test.example` — add E2E*{role}\_EMAIL / E2E*{role}\_PASSWORD entries
