...
> **✅ ARCHIVED — v6.6 shipped 2026-06-13.** All requirements below were implemented and live-verified on staging across phases 62-67 (each per-phase VERIFICATION.md = passed). Outcomes: all validated as written; no requirement dropped or materially adjusted. PERENG-01 implemented as Option B (read branch); PERENG-02 substituted the canonical `engagement_participants` plane for the legacy `person_engagements` (dead, zero writers) per the requirement's 'chosen contract' clause.
