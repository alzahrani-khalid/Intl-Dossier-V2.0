...
GATEDRILL_CLOSE_EXIT=0  gateCount: 24        parse: 24/24 PARSE-OK
CLOSING RUN (after Tasks 1-2 landed):                green 24 / red 0
```

**The two runs together are the mechanical both-directions observation for this plan's own two
gates**: 95-09_g2 and 95-09_g3 were red in the baseline run against a tree where the register and
the decision record did not exist, and green in the closing run against the tree that carries
them. No other gate changed colour between the two runs — the other 22 were green in both, so the
closing green is not a tree that drifted under them.
...
| 95-09_g2  | exit **1** in the baseline drill above — `95-CLOSING-REGISTER.md` absent                                                                                                                                                                                                                     | this file; CLOSING RUN green (§4 head)                                                                                                                                  | —                                                                                                                                                                                                                 | SOUND                                                      |
| 95-09_g3  | exit **1** in the baseline drill above — decision file absent, all eight rows `Pending`                                                                                                                                                                                                      | Task 2's records + row flips; CLOSING RUN green (§4 head)                                                                                                               | C7: the out-of-phase guard is anchored to `phase-95-base`                                                                                                                                                         | SOUND                                                      |
