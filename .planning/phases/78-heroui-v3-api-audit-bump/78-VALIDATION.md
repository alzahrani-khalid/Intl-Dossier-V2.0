...
| ------------------------------------------------------------------------------------ | --------------- | ----------- | ------------------------------------------------------------------- | --------------------------------------------------------------- | --------- | ------------------------------------------------------------------------------------------------------------------------- | ------------------------- | ---------- |
| Lockfile resolves BOTH `@heroui/react` + `@heroui/styles` to 3.2.1, no 3.0.5 remnant | 01 (bump)       | HEROUI-01   | T-78-01 (supply-chain: pinned coupled versions, lockfile integrity) | Only audited 3.2.1 resolves; no unexpected transitive downgrade | script    | `grep -E "@heroui/(react\|styles)@" pnpm-lock.yaml` (root lockfile — no `frontend/pnpm-lock.yaml` exists) → only `@3.2.1` | ✅ (lockfile)             | ⬜ pending |
