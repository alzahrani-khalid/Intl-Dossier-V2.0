...
| REMED-02 | New clearance-gated semantic RPC is `SECURITY INVOKER`                      | T-EoP-DEFINER | `prosecdef = false`                              | integration                    | `SELECT prosecdef FROM pg_proc WHERE proname='search_semantic_clearance_gated'` → false          | ❌ Wave 0   | ⬜ pending |
...
| REMED-05 | Phoenix container responds; Langfuse trace captured                         | —             | Self-hosted, zero egress                         | smoke (post-deploy) / manual   | `curl http://localhost:6006` exits 0 (requires deployed containers); trace in Langfuse UI manual | manual      | ⬜ pending |
...
- [x] All tasks have `<automated>` verify or Wave 0 dependencies (REMED-05 reclassified to post-deploy/manual — no longer a false Wave-0 stub)
- [x] Sampling continuity: no 3 consecutive tasks without automated verify
- [x] Wave 0 covers all MISSING references (5 stubs cover REMED-01/02/03/04/06; REMED-05 is post-deploy smoke)
- [x] No watch-mode flags
- [x] Feedback latency < 90s
- [x] `nyquist_compliant: true` set in frontmatter

**Approval:** pending (set to approved upon phase completion)
