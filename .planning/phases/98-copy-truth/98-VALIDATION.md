...
| COPY-02         | no raw key renders on driven surfaces, both locales (console leg DROPPED rev 1: `saveMissing: false` at HEAD — i18next never invokes `missingKeyHandler`, so a warn-capture leg cannot fire) | e2e DOM detector + census backstop | `98-copy02-rawkeys.spec.ts`                                             | ❌ Wave 0                 |
