...
**v6.6 Dossier Workflow Completion — ALL 6 phases complete (2026-06-13).** Phase 67 (final) wired per-type Engagements tabs to canonical tables: an org Hosted-engagements section on engagement_dossiers.host_organization_id, a person/EO Participation section on engagement_participants (with the get_person_full recent_engagements RPC repointed off the dead person_engagements plane and the wizard created_by RLS write-drop fixed), and deleted ~42 dead legacy \*DossierDetail files (~9,200 lines). Live-verified on staging (seed→observe→restore, EN+AR), exhaustively code-reviewed (0 critical; 2 warnings + grant least-privilege hardening fixed). Awaiting final PR merge, then milestone close.
