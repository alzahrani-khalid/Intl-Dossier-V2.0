...
**In progress:** v6.6 Dossier Workflow Completion — Phases 62 (export pack contract+deploy), 63 (relationship graph route + bidirectional traversal), and 64 (New Position from dossier) complete as of 2026-06-12; 62-63 shipped via PR #56. Phase 64 closed the P0 positions INSERT RLS drift (staging had NO insert policy — deny-all), extracted a working NewPositionDialog with two-step create→applies_to-link submit, rewired both entry points, and live-verified both roadmap criteria on staging (DB link rows + no-reload tab refresh, AR/RTL contract). Phases 65-67 remain.
...
_Last updated: 2026-06-12 — v6.6 phases 62-64 complete (export pack, relationship graph, new-position-from-dossier); phases 65-67 remain. v7.0 Intelligence Engine follows._
