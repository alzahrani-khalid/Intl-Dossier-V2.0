...
## Last Shipped Milestone: v8.0 Linear Design System Migration

**Status: SHIPPED 2026-07-04** — 6 phases (75–80), 32 plans. Audit **passed**: 24/24 requirements, 6/6 phases, cross-phase integration + E2E flows proven by Phase 80, `threats_open: 0` (Phase 80 SECURED 20/20). See `milestones/v8.0-MILESTONE-AUDIT.md`.

**Goal:** Replace the multi-direction IntelDossier/Bureau design language (and the accent-hue axis) with a single Linear-derived visual direction — a re-skin of the existing OKLCH token engine, not a rebuild — on consolidated shadcn RTL infrastructure bridged into Radix, with HeroUI bumped 3.0.5 → 3.2.1 and Aceternity fully removed, preserving Arabic RTL on all four axes (dark/light × LTR/RTL).

**Delivered:** Linear is now the sole visual direction — dark-canonical + derived-light tokens wired bootstrap→tokens→primitives, dual-layer `id.dir`→linear coercion (no legacy profile loses first paint), three-copy byte-match CI guard; the 4-direction + accent-hue switchers retired; the RTL direction source consolidated into one owner bridged into Radix; HeroUI at 3.2.1; Aceternity fully gone (1 live component rebuilt on HeroUI v3/Radix + 7 dead deleted). The whole migration is proven by a human-approved full-route visual re-compare (43 baselines, 0 regressions), an honest recorded axe baseline across all 4 axes, and RTL portal/component smoke tests in CI. Design source-of-truth (DESIGN.md + 3× CLAUDE.md) repointed off Bureau to Linear.

**Known deferred at close (non-blocking):** `test-rtl-smokes` branch-protection promotion (repo-admin) + the CI "birth certificate" run, both deferred to the v8.0 PR (main is local-only ahead; local green is the phase-exit proof); IN-04 UserPicker PostgREST-filter interpolation (pre-existing, RLS-bounded) recorded for a future security pass; the pre-existing `TEST_USER_PASSWORD` staging-cred literal flagged for a credential-hygiene sweep.

<details>
<summary>Prior milestone: v7.0 Intelligence Engine (shipped 2026-06-24)</summary>

**Status: SHIPPED 2026-06-24** — 7 phases (68–74), 49 plans. Audit `gaps_found` with 0 code blockers; EVAL-01/02/03 + AGENT/INFRA live verification deploy-gated (deferred — see `milestones/v7.0-MILESTONE-AUDIT.md`).
...
**Architecture (locked — Option A):** Mastra (TS-native agent runtime) + CopilotKit/AG-UI + vLLM-served local models (Gemma 4 12B, eval-gated & swappable) + pgvector hybrid retrieval (HNSW + tsvector/pg_trgm RRF + local bge-reranker). **Security keystone:** JWT propagation so RLS enforces `sensitivity_level <= clearance` automatically; service-role reserved strictly for cron/no-user paths. Fully on-prem, no data egress. Full detail: `milestones/v7.0-ROADMAP.md`.
...
_No active milestone — v8.0 shipped 2026-07-04. Run `/gsd:new-milestone` to scope the next one. Carried-forward candidates are listed under `## Next Milestone` above; a fresh `.planning/REQUIREMENTS.md` is created at the next milestone kickoff._
