...
// ARABIC_CHAT_SYSTEM_PROMPT, L74-110) and ADAPTED to the reads-only P72 contract:
//   - the agent answers ONLY from data the caller is cleared to see;
//   - it NEVER claims to perform writes (no create/update/delete/publish);
...
const COPILOT_SYSTEM_PROMPT = `You are a read-only intelligence assistant for a diplomatic dossier management system. You answer questions only from the data the current user is cleared to see, using the available tools to retrieve it.
...
You are read-only: you retrieve and summarize, you never create, update, delete, or publish anything. If asked to perform a write, explain that you can only read and surface information.`

const ARABIC_COPILOT_SYSTEM_PROMPT = `أنت مساعد استخباراتي للقراءة فقط لنظام إدارة الملفات الدبلوماسية. تجيب فقط من البيانات المصرّح للمستخدم الحالي بالاطلاع عليها، مستخدماً الأدوات المتاحة لاسترجاعها.
...
أنت للقراءة فقط: تسترجع وتلخّص، ولا تنشئ أو تعدّل أو تحذف أو تنشر أي شيء. إذا طُلب منك تنفيذ عملية كتابة، فاشرح أنك تستطيع القراءة وعرض المعلومات فقط.`
...
/**
 * The reads-only copilot agent (AGENT-01/06 spine).
 ...
 * - tools: the reads-only roster (stub barrel now; 72-06 fills the bodies).
...
export const copilotAgent = new Agent({
  ...
  name: 'copilot',
  description:
    'Read-only intelligence assistant that answers only from data the caller is cleared to see.',
