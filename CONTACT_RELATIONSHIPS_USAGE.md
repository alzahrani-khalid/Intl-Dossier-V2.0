...
```tsx
const collaborators = relationships.filter(
  r => r.relationship_type === 'collaborates_with'
);
```
...
const secondHopQueries = firstHop.map(rel => 
  useRelationships(rel.to_contact_id)
);
```
