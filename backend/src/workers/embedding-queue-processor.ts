...
export class EmbeddingQueueProcessor {
  ...
  start(): void {
    if (this.processingInterval) {
      console.warn('Embedding queue processor already running')
      return
    }

    console.warn('Starting embedding queue processor (every 30 seconds)')
  ...
  stop(): void {
    if (this.processingInterval) {
      ...
      console.warn('Embedding queue processor stopped')
    }
  }
  ...
  private async processQueue(): Promise<void> {
    if (this.isProcessing) {
      console.warn('Queue processing already in progress, skipping...')
      return
    }
    ...
    try {
      ...
      if (!queueItems || queueItems.length === 0) {
        console.warn('Embedding queue is empty')
        return
      }

      console.warn(`Processing ${queueItems.length} embedding queue items...`)
      ...
      console.warn(
        `Queue processing complete: ${successCount} succeeded, ${failureCount} failed (${tookMs}ms)`,
      )
    } catch (error) {
  ...
  private async processQueueItem(item: QueueItem): Promise<void> {
    try {
      ...
      await this.supabase.from('embedding_update_queue').delete().eq('id', item.id)

      console.warn(`✓ Processed embedding for ${item.entity_type}/${item.entity_id}`)
    } catch (error) {
  ...
  private async updateEntityEmbedding(
    ...
    const { error } = await this.supabase.from(tableName).update({ embedding }).eq('id', entityId)
  ...
  async getQueueStats(): Promise<{
    ...
    const avgRetryCount =
      pending > 0 ? pendingItems.reduce((sum, item) => sum + item.retry_count, 0) / pending : 0
