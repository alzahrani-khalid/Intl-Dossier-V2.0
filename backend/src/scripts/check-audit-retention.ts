...
async function checkAuditRetention() {
  console.warn('🔍 Checking audit log retention compliance...\n')
  ...
  try {
    ...
    if (!eligibleLogs || eligibleLogs.length === 0) {
      console.warn('✅ No audit logs older than 7 years found.')
      console.warn('   Retention policy is in compliance.\n')
      return
    }

    // Alert: Logs found that need archival
    console.warn(
      `⚠️  WARNING: ${eligibleLogs.length} audit logs are older than 7 years and need archival!\n`,
    )
    ...
    console.warn('📊 Age Distribution:')
    Object.entries(ageRanges).forEach(([range, count]) => {
      if (count > 0) {
        console.warn(`   ${range}: ${count} logs`)
      }
    })

    console.warn('\n📋 Oldest 10 logs:')
    eligibleLogs.slice(0, 10).forEach((log: ArchivalEligibleLog) => {
      console.warn(`   - ID: ${log.id}, Age: ${log.age}, Action: ${log.action}`)
    })

    console.warn('\n📖 Next Steps:')
    console.warn('   1. Review archival policy in docs/compliance/audit-log-retention.md')
    console.warn('   2. Archive logs to cold storage (S3 Glacier, Azure Archive, etc.)')
    console.warn('   3. Verify archival backup before deletion')
    console.warn('   4. Document archival process with timestamps\n')
