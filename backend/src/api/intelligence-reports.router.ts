...
} from '../models/intelligence-report.model'
...
const vectorService = new VectorService(process.env.SUPABASE_URL!, process.env.SUPABASE_ANON_KEY!)
...
router.get('/', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    let query = supabase.from('intelligence_reports').select('*', { count: 'exact' })
...
router.post('/search', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { query, filters, similarity_threshold = 0.8, timeout_ms = 2000 } = req.body
