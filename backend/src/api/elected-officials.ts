 ...
 * @access Private
 */
router.get('/', validate({ query: electedOfficialFiltersSchema }), async (req, res, next) => {
  try {
    const {
      search,
      office_type,
  ...
  async (req, res, next) => {
    try {
      ...
      if (Object.keys(dossierFields).length > 0) {
        ...
        if (dossierError != null) {
          logError(
            'Failed to update dossier for elected official',
            dossierError as unknown as Error,
          )
          return res.status(500).json({ error: dossierError.message })
        }
      }
