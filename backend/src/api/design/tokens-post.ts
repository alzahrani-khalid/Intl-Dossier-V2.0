...
export async function createOrUpdateDesignToken(req: Request, res: Response) {
  try {
    ...
    if (!cssVariablePattern.test(token.cssVariable)) {
      return res.status(400).json({
        error:
          'CSS variable must start with -- and contain only lowercase letters, numbers, and hyphens',
      })
    }
    ...
    if (existingToken) {
      const { error } = await supabase.from('design_tokens').update(tokenData).eq('id', token.id)
