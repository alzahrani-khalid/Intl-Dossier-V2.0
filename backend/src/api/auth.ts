  ...
  try {
    ...
    if (result.requiresMFA && !result.accessToken) {
      return res.status(200).json({
        ...
        message: createBilingualError('MFA code required', 'رمز المصادقة الثنائية مطلوب', lang),
      })
    }
    ...
    res.json({
      ...
      message: verified
        ...
        : createBilingualError(
            ...
            'فشل التحقق من المصادقة الثنائية',
            getRequestLanguage(req),
          ),
    })
  } catch (error) {
  ...
  try {
    ...
    if (!token) {
      return res.status(401).json({
        error: createBilingualError('Authentication required', 'المصادقة مطلوبة', lang),
      })
    }
