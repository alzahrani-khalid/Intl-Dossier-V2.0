...
async function initializeWhisper(): Promise<typeof whisperModel> {
  if (!whisperModel) {
    whisperModel = await pipeline('automatic-speech-recognition', 'openai/whisper-base', {
      quantized: true,
    })
  }
  return whisperModel
 ...
 * @access Private
 */
router.get('/commands', authenticate, async (req: Request, res: Response) => {
  ...
  const commands =
    language === 'ar'
      ? {
          ...
          search: [{ command: 'ابحث عن [الكلمة]', description: 'البحث في النظام' }],
          actions: [
...
async function executeVoiceCommand(command: any, _userId?: string): Promise<any> {
