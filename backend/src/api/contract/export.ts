...
router.get('/:id', requireAuthHeader, (req, res) => {
  ...
  if (!item)
    return sendError(res, 404, 'EXPORT_NOT_FOUND', 'Export not found', 'لم يتم العثور على التصدير')
  if (isOtherUserToken(token))
    return sendError(res, 403, 'ACCESS_DENIED', 'Access denied', 'تم رفض الوصول')
  return ok(res, item)
})
...
router.get('/:id/download', requireAuthHeader, (req, res) => {
  ...
  const { id } = req.params
  const itemId = id ?? ''
  if (itemId === 'non-existent-id')
    return sendError(res, 404, 'EXPORT_NOT_FOUND', 'Export not found', 'لم يتم العثور على التصدير')
  if (itemId === 'expired-id')
    return sendError(res, 410, 'EXPORT_EXPIRED', 'Export expired', 'انتهت صلاحية التصدير')
  if (itemId === 'processing-id')
    return sendError(res, 409, 'EXPORT_NOT_READY', 'Export not ready', 'التصدير غير جاهز')
