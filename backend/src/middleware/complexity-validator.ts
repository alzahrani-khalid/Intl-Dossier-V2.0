 ...
 */
export function validateGraphQuery(params: { depth?: number; maxNodes?: number }): void {
 ...
 */
export function validateSearchQuery(params: { query?: string; limit?: number }): void {
...
export function validateCalendarQuery(params: {
  ...
  if (isNaN(start.getTime()) || isNaN(end.getTime())) {
    throw new ComplexityBudgetError('Invalid date format in calendar query', 0, 0, 'dateFormat')
  }
 ...
 */
export function validatePagination(params: { page?: number; pageSize?: number }): void {
  const { page = 1, pageSize = COMPLEXITY_LIMITS.DEFAULT_PAGE_SIZE } = params

  if (page < 1) {
    throw new ComplexityBudgetError('Page number must be at least 1', 1, page, 'page')
  }
  ...
  if (pageSize < 1) {
    throw new ComplexityBudgetError('Page size must be at least 1', 1, pageSize, 'pageSize')
  }
}
