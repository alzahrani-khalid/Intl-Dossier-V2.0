...
async function searchEntityType(
  ...
  const results: EntitySearchResult[] = entities.map((entity: any) => {
    ...
    const description = searchConfig.descriptionExtractor
      ? searchConfig.descriptionExtractor(entity)
      : undefined
    ...
    return {
      ...
      name,
      description,
      classification_level: searchConfig.classificationField
        ? Number(entity[searchConfig.classificationField])
        : undefined,
      last_linked_at: (entity.last_linked_at || entity.updated_at || entity.created_at) as
        | string
        | undefined,
...
function getEntitySearchConfig(entityType: EntityType) {
  const configs: Record<EntityType, any> = {
    ...
    position: {
      table: 'positions',
      selectFields:
        'id, title_en, title_ar, description_en, description_ar, classification_level, archived_at, created_at, updated_at',
    ...
    assignment: {
      table: 'assignments',
      selectFields:
        'id, title, description, classification_level, archived_at, created_at, updated_at',
      searchFields: ['title', 'description'],
      nameExtractor: (e: any) => e.title,
      descriptionExtractor: (e: any) => e.description,
      archivedField: 'archived_at',
...
export async function getEntityMetadata(
  ...
  const metadata: EntityMetadata = {
    ...
    description: searchConfig.descriptionExtractor
      ? searchConfig.descriptionExtractor(entity)
      : undefined,
    classification_level: searchConfig.classificationField
      ? Number(entity[searchConfig.classificationField])
    ...
    last_linked_at: (entity.last_linked_at || entity.updated_at || entity.created_at) as
      ...
      | undefined,
    is_archived:
      searchConfig.archivedField === 'status'
        ? entity.status === 'archived'
        : entity[searchConfig.archivedField] !== null,
    organization_id: searchConfig.organizationField
      ? (entity[searchConfig.organizationField] as string)
      : '',
  }
