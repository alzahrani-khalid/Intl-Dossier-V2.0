    ...
    try {
      ...
      if (!type || type === 'all') {
        ...
        const [countries, orgs, mous] = await Promise.all([
          supabaseAdmin.from('countries').select('name_en').ilike('name_en', `${query}%`).limit(5),
          supabaseAdmin
            .from('organizations')
            .select('name_en')
            .ilike('name_en', `${query}%`)
            .limit(5),
          supabaseAdmin.from('mous').select('title_en').ilike('title_en', `${query}%`).limit(5),
        ])
        ...
        if (data) {
          suggestions.push(...data.map((item: any) => item[field]))
        }
      }
