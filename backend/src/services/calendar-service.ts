...
export class CalendarService {
  ...
  async createCalendarEvent(input: CreateCalendarEventInput) {
    ...
    if (input.is_virtual && !input.virtual_link) {
      throw new Error(
        'Cannot create virtual event: virtual_link is required when is_virtual = true',
      )
    }
  ...
  async getEventParticipants(
    ...
    let query = this.supabase.from('event_participants').select('*').eq('event_id', eventId)
   ...
   */
  async updateParticipantStatus(participantId: string, attendanceStatus: AttendanceStatus) {
    const { data, error } = await this.supabase
      .from('event_participants')
  ...
  async getUpcomingEvents(
    ...
    return this.getEventsInDateRange(now.toISOString(), future.toISOString(), options)
  }
  ...
  async getParticipantStats(eventId: string) {
    ...
    data?.forEach((participant) => {
      ...
      stats.by_status[participant.attendance_status] =
        (stats.by_status[participant.attendance_status] || 0) + 1
    })
