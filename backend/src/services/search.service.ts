...
export class SearchService {
  ...
  constructor(supabaseUrl: string, supabaseKey: string, vectorService: VectorService) {
  ...
  private async loadSearchFilters(): Promise<void> {
    const { data, error } = await this.supabase.from('search_filters').select('*')
  ...
  private async searchWithKeywords(options: SearchOptions): Promise<IntelligenceReport[]> {
    ...
    if (options.query) {
      query = query.or(
        `title.ilike.%${options.query}%,content.ilike.%${options.query}%,title_ar.ilike.%${options.query}%,content_ar.ilike.%${options.query}%`,
      )
    }
  ...
  private async searchWithFilters(
    ...
    const queryPromise = this.supabase.from('intelligence_reports').select('*').match(filterQuery)
  ...
  private async filterByPriority(
    ...
    return reports.filter((r) => {
      const highSeverityCount =
        r.threat_indicators?.filter((t) => t.severity === 'high' || t.severity === 'critical')
          .length || 0

      if (
        priorities.includes('critical') &&
        r.threat_indicators?.some((t) => t.severity === 'critical')
      ) {
        return true
      }
      ...
      if (
        ...
        r.threat_indicators?.some((t) => t.severity === 'medium')
      ) {
        return true
      }
  ...
  async deleteSearchFilter(id: string): Promise<void> {
    const { error } = await this.supabase.from('search_filters').delete().eq('id', id)
