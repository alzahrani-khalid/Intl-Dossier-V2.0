...
export class MoUService {
  ...
  private readonly stateTransitions: MoUTransition[] = [
    ...
    { from_status: 'draft', to_status: 'terminated', allowed: true },
    {
      ...
      to_status: 'signed',
      allowed: true,
      required_fields: ['sign_date', 'parties'],
    },
  ...
  async getMoUs(params: MoUSearchParams = {}): Promise<{
    ...
    try {
      ...
      // Build query
      let queryBuilder = supabaseAdmin.from('mous').select('*', { count: 'exact' })
      ...
      if (has_deliverables_due && filteredData.length > 0) {
        ...
        filteredData = filteredData.filter((mou) =>
          mou.deliverables?.some(
            (d: any) => d.status !== 'completed' && new Date(d.due_date) <= now,
          ),
        )
      }
  ...
  async getMoUById(id: string): Promise<MoU | null> {
    try {
      ...
      const { data, error } = await supabaseAdmin.from('mous').select('*').eq('id', id).single()
  ...
  async calculateMoUPerformance(mouId: string): Promise<{
    ...
    try {
      ...
      const deliverables_completion =
        deliverables.length > 0 ? (completedDeliverables.length / deliverables.length) * 100 : 0

      // Calculate on-time delivery
      const completedOnTime = completedDeliverables.filter(
        (d) => d.completion_date && new Date(d.completion_date) <= new Date(d.due_date),
      )
      ...
      const metricsAchieved = metrics.filter((m) => m.current_value >= m.target_value)
      const metrics_achievement =
        metrics.length > 0 ? (metricsAchieved.length / metrics.length) * 100 : 0

      // Calculate overall score
      const overall_score =
        deliverables_completion * 0.4 + on_time_delivery * 0.3 + metrics_achievement * 0.3
