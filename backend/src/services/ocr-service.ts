...
export class OCRService {
  ...
  async preprocessImage(buffer: Buffer): Promise<Buffer> {
    try {
      ...
      console.warn(
        `Preprocessed image: ${metadata.width}x${metadata.height} -> ${resizeOptions.width || metadata.width}px`,
      )
  ...
  async extractTextTesseract(imagePath: string | Buffer): Promise<OCRResult> {
    try {
      ...
      const imageData = typeof imagePath === 'string' ? await fs.readFile(imagePath) : imagePath
      ...
      const averageConfidence =
        words.length > 0 ? words.reduce((sum, word) => sum + word.confidence, 0) / words.length : 0
  ...
  async extractTextGoogleVision(imagePath: string | Buffer): Promise<OCRResult> {
    if (!this.visionClient) {
      throw new Error(
        'Google Vision API client not configured. Please set GOOGLE_APPLICATION_CREDENTIALS environment variable.',
      )
    }
    ...
    try {
      ...
      throw new Error(
        `Google Vision extraction failed: ${error instanceof Error ? error.message : 'Unknown error'}`,
      )
    }
  }
  ...
  async extractBusinessCard(
    ...
    try {
      ...
      console.warn('Attempting OCR with Tesseract (local)...')
      result = await this.extractTextTesseract(imagePath)

      console.warn(`Tesseract confidence: ${result.confidence}%`)
      ...
      if (result.confidence < this.CONFIDENCE_THRESHOLD) {
        if (consentCloudOCR && this.visionClient) {
          console.warn('Low confidence, attempting Google Vision (cloud)...')
          try {
            ...
            if (visionResult.confidence > result.confidence) {
              console.warn(`Google Vision confidence: ${visionResult.confidence}% (using this)`)
              result = visionResult
            } else {
              console.warn(
                `Google Vision confidence: ${visionResult.confidence}% (keeping Tesseract)`,
              )
            }
          } catch (visionError) {
          ...
          console.warn('Cloud OCR not consented, using Tesseract result despite low confidence')
        } else {
          console.warn('Google Vision not available, using Tesseract result')
        }
      }
      ...
      throw new Error(
        `Business card extraction failed: ${error instanceof Error ? error.message : 'Unknown error'}`,
      )
    }
  }
  ...
  parseContactFields(rawText: string): ExtractedContact {
    ...
    const titleKeywords =
      /\b(CEO|CTO|CFO|COO|Director|Manager|Engineer|Developer|Designer|Consultant|President|Vice President|VP|Head|Lead|Senior|Jr\.|Sr\.|Dr\.|Prof\.|Mr\.|Mrs\.|Ms\.)\b/i
    const organizationKeywords =
      /\b(Company|Corporation|Corp\.|Inc\.|LLC|Ltd\.|Limited|Group|Institute|University|College|Foundation|Association|Ministry|Department)\b/i
    ...
    let fieldCount = 0
    const totalFields = 5 // name, organization, position, email, phone
