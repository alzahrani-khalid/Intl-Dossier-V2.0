...
import { getRedisCacheService, RedisCacheService } from './redis-cache.service'
...
export class SuggestionService {
  ...
  async getSuggestions(options: SuggestionOptions): Promise<SuggestionResponse> {
    ...
    const cacheKey = RedisCacheService.generateSuggestionKey(entityType, normalizedPrefix, language)
  ...
  private async querySingleEntityType(
    ...
    try {
      ...
      return data.map((row: any, index: number) => {
        ...
        const score = this.calculateSimilarityScore(prefix, language === 'ar' ? titleAr : titleEn)
