...
export class CountrySearchService {
  ...
  async search(options: CountrySearchOptions): Promise<CountrySearchResult> {
    ...
    let query = this.supabase.from('countries').select('*', { count: 'exact' })
  ...
  async searchByText(
    ...
    const filters: CountrySearchFilters =
      language === 'en' ? { name: searchText } : { nameAr: searchText }
  ...
  async bulkSearch(
    isoCodes: string[],
  ): Promise<Database['public']['Tables']['countries']['Row'][]> {
    const upperCodes = isoCodes.map((code) => code.toUpperCase())

    const { data, error } = await this.supabase
      .from('countries')
      .select('*')
      .or(`iso_code_2.in.(${upperCodes.join(',')}),iso_code_3.in.(${upperCodes.join(',')})`)
