...
export class ClusteringService {
  run(req: ClusteringRequest): ClusteringResult {
    if (!req.dataset_id)
      throw Object.assign(new Error('Missing dataset_id'), {
        code: 'MISSING_DATASET_ID',
        status: 400,
      })
    if (!req.data)
      throw Object.assign(new Error('Missing data'), { code: 'MISSING_DATA', status: 400 })
    if (!Array.isArray(req.data) || req.data.length === 0)
      throw Object.assign(new Error('Empty data'), { code: 'EMPTY_DATA', status: 400 })
    ...
    if (req.dataset_id === 'restricted-dataset')
    ...
    if (k < 3 || k > req.data.length)
      throw Object.assign(new Error('Invalid cluster_count'), {
        code: 'INVALID_CLUSTER_COUNT',
        status: 400,
      })
    if (req.data.length < 3)
      throw Object.assign(new Error('Insufficient data points'), {
