...
export class PositionConsistencyService {
  ...
  async analyzeConsistency(thematicAreaId: string): Promise<ConsistencyAnalysisResult> {
  ...
  private async detectConflicts(positions: any[]): Promise<PositionConflict[]> {
    ...
    for (let i = 0; i < positions.length - 1; i++) {
      for (let j = i + 1; j < positions.length; j++) {
        const conflict = await this.analyzePositionPair(positions[i], positions[j])
  ...
  private calculateScore(totalPositions: number, conflicts: PositionConflict[]): number {
  ...
  private generateRecommendations(conflicts: PositionConflict[]): string[] {
    ...
    if (outdated.length > 0) {
      recommendations.push(`Update ${outdated.length} outdated positions to reflect current policy`)
    }
  ...
  private async saveAnalysis(
    ...
    const { error } = await this.supabase.from('position_consistency').insert({
