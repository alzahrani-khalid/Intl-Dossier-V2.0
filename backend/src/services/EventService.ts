...
export class EventService {
  ...
  async findAll(params: EventSearchParams = {}): Promise<{ data: Event[]; total: number }> {
    try {
      ...
      let query = supabaseAdmin.from('event_details').select('*')
      ...
      // Apply sorting
      const sortField =
        params.sort === 'start_date' || !params.sort ? 'start_datetime' : params.sort
  ...
  async create(eventData: CreateEventDto, createdBy: string): Promise<Event> {
    try {
      ...
      const location =
        (eventData as any).venue_en ||
        (eventData as any).location_en ||
  ...
  async update(id: string, updates: UpdateEventDto, updatedBy: string): Promise<Event> {
    try {
      ...
      const location =
        (updates as any).venue_en || (updates as any).location_en || (updates as any).location
  ...
  async addAttendee(
    ...
    try {
      ...
      // Invalidate cache
      await cacheHelpers.del([`${this.cachePrefix}${eventId}`, `${this.cachePrefix}list:*`])
   ...
   */
  async removeAttendee(eventId: string, attendeeId: string, removedBy: string): Promise<Event> {
    try {
  ...
  async checkConflicts(
    ...
    try {
      let query = supabaseAdmin.from('events').select('*').or(`
