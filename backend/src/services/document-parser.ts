...
export class DocumentParser {
  ...
  private readonly ARABIC_NAME_PREFIXES = [
    'د.',
    'م.',
    'أ.',
    'السيد',
  ...
  private readonly ORG_KEYWORDS_AR = [
    'شركة',
    'مؤسسة',
    'جامعة',
    'كلية',
  ...
  private readonly ORG_KEYWORDS_EN = [
    'Company',
    'Corporation',
    'Corp.',
    'Inc.',
  ...
  private readonly POSITION_KEYWORDS_AR = [
    'مدير',
    'رئيس',
    'نائب',
    'مساعد',
  ...
  private readonly POSITION_KEYWORDS_EN = [
    'CEO',
    'CTO',
    'CFO',
    'COO',
  ...
  extractContacts(text: string, language: string = 'en'): ExtractedContact[] {
    ...
    for (let i = 0; i < blocks.length; i++) {
      ...
      if (
        contact &&
        (contact.full_name || contact.email_addresses?.length || contact.phone_numbers?.length)
      ) {
  ...
  private isLikelyContactStart(line: string): boolean {
    ...
    const hasTitle = [...this.ARABIC_NAME_PREFIXES, ...this.ENGLISH_TITLES].some((title) =>
    ...
    const isTitleCase =
      words.length <= 5 && words.every((w) => /^[A-Z]/.test(w) || /^[\u0600-\u06FF]/.test(w))
  ...
  private hasContactInfo(text: string): boolean {
    ...
    const hasOrgKeyword = [...this.ORG_KEYWORDS_AR, ...this.ORG_KEYWORDS_EN].some((keyword) =>
   ...
   */
  private extractContactFromBlock(
    block: string,
    language: string,
    blockNumber: number,
  ): ExtractedContact {
