...
export class AnomalyDetectionService {
  list(params: {
    ...
    if (typeof params.min_score === 'number')
      items = items.filter((a) => a.anomaly_score >= params.min_score!)
  ...
  review(id: string, classification?: string, false_positive?: boolean) {
    ...
    if (item.reviewed_at)
      throw Object.assign(new Error('Already reviewed'), {
        code: 'ANOMALY_ALREADY_REVIEWED',
        status: 409,
      })
    if (!classification)
      throw Object.assign(new Error('Missing classification'), {
        code: 'MISSING_CLASSIFICATION',
        status: 400,
      })
    if (typeof false_positive !== 'boolean')
      throw Object.assign(new Error('Missing false_positive'), {
        code: 'MISSING_FALSE_POSITIVE_FLAG',
        status: 400,
      })
    const valid = ['legitimate', 'suspicious', 'malicious']
    if (!valid.includes(classification))
      throw Object.assign(new Error('Invalid classification'), {
