...
أرجع JSON بهذا التنسيق بالضبط:
{
  ...
  private parseLLMResponse(
    responseText: string,
    language: 'en' | 'ar',
  ): Omit<ExtractionResponse, 'extraction_id' | 'processing_time_ms'> {
    try {
  ...
  private async validateExtractions(
  ...
  ): Promise<Omit<ExtractionResponse, 'extraction_id' | 'processing_time_ms'>> {
    ...
    data.commitments = data.commitments.map((c) => {
      ...
      if (dueDate < today) {
        ...
        logger.warn('Commitment has past due date', {
    ...
    data.follow_up_actions = data.follow_up_actions.filter(
      (a) => a.confidence_score >= minConfidence,
    )
  ...
  private findBestOwnerMatch(
    description: string,
    historicalData: any[],
  ): { owner_name: string; similarity: number } | null {
    ...
    for (const historical of historicalData) {
      const historicalKeywords = historical.description
        .toLowerCase()
        .split(/\s+/)
