...
export class DossierService {
   ...
   */
  async createCountryDossier(input: CreateDossierBase & {
    iso_code_2: string;
    iso_code_3: string;
    capital_en?: string;
    capital_ar?: string;
    ...
    const { iso_code_2, iso_code_3, capital_en, capital_ar, region, subregion, population, area_sq_km, flag_url, ...baseData } = input;
   ...
   * @returns Created dossier with extension data
   */
  async createOrganizationDossier(input: CreateDossierBase & {
    org_code?: string;
    org_type: 'government' | 'ngo' | 'private' | 'international' | 'academic';
    headquarters_country_id?: string;
    parent_org_id?: string;
    ...
    const {
      org_code, org_type, headquarters_country_id, parent_org_id,
      website, email, phone, address_en, address_ar, logo_url, established_date,
      ...baseData
    } = input;
   ...
   * @returns Created dossier with extension data
   */
  async createForumDossier(input: CreateDossierBase & {
    number_of_sessions?: number;
    keynote_speakers?: Array<{ name: string; title: string; org: string }>;
    sponsors?: Array<unknown>;
    registration_fee?: number;
    ...
    const {
      number_of_sessions, keynote_speakers, sponsors,
      registration_fee, currency, agenda_url, live_stream_url,
      ...baseData
    } = input;
   ...
   * @returns Created dossier with extension data
   */
  async createEngagementDossier(input: CreateDossierBase & {
    engagement_type: 'meeting' | 'consultation' | 'coordination' | 'workshop' | 'conference' | 'site_visit' | 'ceremony';
    engagement_category: 'bilateral' | 'multilateral' | 'regional' | 'internal';
    location_en?: string;
    location_ar?: string;
    ...
    const {
      engagement_type, engagement_category, location_en, location_ar,
      ...baseData
    } = input;
   ...
   * @returns Created dossier with extension data
   */
  async createThemeDossier(input: CreateDossierBase & {
    theme_category: 'policy' | 'technical' | 'strategic' | 'operational';
    parent_theme_id?: string;
  }) {
   ...
   * @returns Created dossier with extension data
   */
  async createWorkingGroupDossier(input: CreateDossierBase & {
    mandate_en?: string;
    mandate_ar?: string;
    lead_org_id?: string;
    wg_status?: 'active' | 'suspended' | 'disbanded';
    ...
    const {
      mandate_en, mandate_ar, lead_org_id, wg_status,
      established_date, disbandment_date,
      ...baseData
    } = input;
   ...
   * @returns Created dossier with extension data
   */
  async createPersonDossier(input: CreateDossierBase & {
    title_en?: string;
    title_ar?: string;
    organization_id?: string;
    nationality_country_id?: string;
    ...
    const {
      title_en, title_ar, organization_id, nationality_country_id,
      email, phone, biography_en, biography_ar, photo_url,
      ...baseData
    } = input;
    ...
    if (organization_id) {
      ...
      if (orgError || !org) {
        throw new Error(`Invalid organization_id: ${organization_id} does not reference a valid organization dossier`);
      }
    }
    ...
    if (nationality_country_id) {
      ...
      if (countryError || !country) {
        throw new Error(`Invalid nationality_country_id: ${nationality_country_id} does not reference a valid country dossier`);
      }
    }
  ...
  /**
   * Get a dossier by ID with its extension data (type-based JOIN)
  ...
  /**
   * List dossiers with optional type filtering and pagination
   * Implements Redis caching with 3-minute TTL (T130 - Performance Optimization)
  ...
  async linkDocumentToDossier(input: {
    ...
    if (error) {
      ...
      if (error.code === '23505') {
        throw new Error(`Link already exists between position ${positionId} and dossier ${dossierId} with type ${linkType}`);
      }
