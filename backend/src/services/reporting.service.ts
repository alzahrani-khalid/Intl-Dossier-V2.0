...
export class ReportingService {
  ...
  private async loadTemplates(): Promise<void> {
    const { data, error } = await this.supabase.from('report_templates').select('*')
  ...
  private async renderTemplate(
    ...
    const templateContent =
      language === 'ar' && template.template_content_ar
  ...
  private formatExecutiveSummary(data: any[]): string {
    const summary = {
      ...
      critical_threats: data.filter((r) =>
        r.threat_indicators?.some((t: any) => t.severity === 'critical'),
      ).length,
      high_confidence: data.filter((r) => r.confidence_score >= 80).length,
      regions_covered: new Set(
        data.flatMap((r) => r.geospatial_tags?.map((g: any) => g.location_type) || []),
      ).size,
    }
