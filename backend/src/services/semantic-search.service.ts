...
export class SemanticSearchService {
  ...
  async search(options: SemanticSearchOptions): Promise<SemanticSearchResponse> {
    ...
    if (entityTypes.length === 0) {
      warnings.push(
        'No valid entity types for semantic search. Valid types: positions, documents, briefs',
      )
      return {
        results: [],
        query: {
  ...
  /**
   * Generate embedding using BGE-M3 (primary) with VectorService fallback
   */
   ...
   */
  getEmbeddingModelInfo(): { model: string; dimensions: number } {
    return embeddingsService.getModelInfo()
  }
}
