...
export class AnalyticsService {
  async getDashboardMetrics(userId: string) {
    try {
      ...
      const metrics = {
        ...
        upcoming_events:
          events.data?.filter((e: any) => new Date(e.start_date) > new Date()).length || 0,
  ...
  async calculateROI(mouId: string) {
    const mou = await supabaseAdmin.from('mous').select('*').eq('id', mouId).single()

    if (!mou.data) throw new Error('MoU not found')

    const completedDeliverables =
      mou.data.deliverables?.filter((d: any) => d.status === 'completed').length || 0
