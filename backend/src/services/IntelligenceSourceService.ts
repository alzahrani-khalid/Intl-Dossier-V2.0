...
export class IntelligenceSourceService {
  ...
  async createSource(dto: CreateIntelligenceSourceDto): Promise<IntelligenceSource> {
  ...
  async getActiveSourcesSummary(): Promise<any> {
    ...
    if (data && data.length > 0) {
      data.forEach((source: any) => {
        ...
        summary.scan_frequencies[scanFreq] = (summary.scan_frequencies[scanFreq] || 0) + 1
      })
  ...
  private filterRelevantItems(items: any[], source: IntelligenceSource): any[] {
    return items.filter((item) => {
      ...
      if (source.keywords.length > 0) {
        const hasKeyword = source.keywords.some((keyword) => text.includes(keyword.toLowerCase()))
        if (!hasKeyword) return false
      }

      // Check categories
      if (source.categories.length > 0 && item.categories) {
        const hasCategory = item.categories.some((cat: string) => source.categories.includes(cat))
  ...
  private async saveIntelligenceItems(items: any[], source: IntelligenceSource): Promise<void> {
    ...
    if (intelligenceItems.length > 0) {
      const { error } = await this.supabase.from('intelligence').insert(intelligenceItems)
  ...
  private async updateSourceAfterScan(
    ...
    if (success) {
      ...
      // Log error
      if (errorMessage) {
        await this.supabase.from('intelligence_source_errors').insert({
          source_id: source.id,
