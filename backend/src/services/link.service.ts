...
export async function createEntityLink(
  ...
  try {
    ...
    if (
      entityMetadata.classificationLevel &&
...
function validateLinkTypeConstraints(
  ...
  if (linkType === 'primary' && !ANCHOR_ENTITY_TYPES.includes(entityType)) {
    errors.push(
      `Link type 'primary' is only allowed for anchor entities: ${ANCHOR_ENTITY_TYPES.join(', ')}`,
    )
  }
  ...
  if (linkType === 'assigned_to' && !ASSIGNABLE_ENTITY_TYPES.includes(entityType)) {
    errors.push(
      `Link type 'assigned_to' is only allowed for: ${ASSIGNABLE_ENTITY_TYPES.join(', ')}`,
    )
  }
...
async function getEntityMetadata(
  ...
  const isArchived =
    entityType === 'dossier' ? entity.status === 'archived' : entity[mapping.archivedField] !== null

  return {
    exists: true,
    isArchived,
    classificationLevel: mapping.classificationField
      ? entity[mapping.classificationField]
      : undefined,
 ...
 */
export async function deleteEntityLink(linkId: string, userId: string): Promise<EntityLink> {
  try {
 ...
 */
export async function restoreEntityLink(linkId: string, userId: string): Promise<EntityLink> {
  try {
...
export async function getEntityIntakes(
  ...
  try {
    ...
    let query = supabaseAdmin
      .from('intake_entity_links')
      .select(
        'intake_id, id, link_type, source, confidence, notes, link_order, linked_by, created_at, updated_at',
        { count: 'exact' },
      )
    ...
    // Add pagination
    query = query.range(offset, offset + pageSize - 1).order('created_at', { ascending: false }) // Most recent first
    ...
    if (options.userClearanceLevel !== undefined) {
      ...
      if (intakeIds.length > 0) {
        ...
        if (intakeData) {
          const intakeClassMap = new Map(intakeData.map((i) => [i.id, i.classification_level || 0]))
