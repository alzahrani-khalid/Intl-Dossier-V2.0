...
import { VectorEmbedding, VectorEmbeddingInput, VectorEmbeddingModel } from '../models/vector-embedding.model';
...
export class VectorService {
  ...
  async createEmbedding(input: VectorEmbeddingInput): Promise<VectorEmbedding> {
    ...
    const { data, error } = await this.supabase
      ...
      .insert({
        ...
        similarity_threshold: input.similarity_threshold || VectorEmbeddingModel.DEFAULT_SIMILARITY_THRESHOLD,
        created_at: new Date(),
        updated_at: new Date()
      })
      .select()
  ...
  private async fallbackSearch(filter: Record<string, any>, limit: number): Promise<VectorSearchResult[]> {
