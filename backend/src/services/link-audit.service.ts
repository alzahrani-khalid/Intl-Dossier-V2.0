...
export async function createAuditLog(
  ...
  try {
    ...
    const { error: auditError } = await supabaseAdmin.from('link_audit_logs').insert({
      link_id: linkId,
...
async function createBatchAuditLogs(
  ...
  try {
    ...
    const { error: auditError } = await supabaseAdmin.from('link_audit_logs').insert(auditEntries)
...
async function getAuditStatistics(
  ...
  try {
    let query = supabaseAdmin.from('link_audit_logs').select('action, performed_by')
