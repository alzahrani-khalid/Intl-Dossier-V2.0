...
export class AlertsService {
  list(filter?: { severity?: string; is_active?: boolean }) {
    ...
    if (typeof filter?.is_active === 'boolean')
      items = items.filter((a) => a.is_active === filter.is_active)
    return items
  }
  get(id: string) {
    return store.get(id)
  ...
  create(
    input: Omit<AlertConfig, 'id' | 'created_at' | 'updated_at' | 'acknowledged'>,
  ): AlertConfig {
    const now = new Date().toISOString()
    const item: AlertConfig = {
      id: rid(),
      acknowledged: false,
      created_at: now,
      updated_at: now,
  ...
  acknowledge(id: string) {
    ...
    if (current.acknowledged)
      throw Object.assign(new Error('Already acknowledged'), {
