...
export class ExportService {
  create(owner: Token, resource_type: string, format: ExportFormat): ExportRequest {
    if (!resource_type)
      throw Object.assign(new Error('Missing resource_type'), {
        code: 'MISSING_RESOURCE_TYPE',
        status: 400,
      })
    if (!format)
      throw Object.assign(new Error('Missing format'), { code: 'MISSING_FORMAT', status: 400 })
    if (!['csv', 'json', 'excel'].includes(format))
      throw Object.assign(new Error('Invalid format'), { code: 'INVALID_FORMAT', status: 400 })
    if (!allowedResources.has(resource_type))
      throw Object.assign(new Error('Invalid resource_type'), {
