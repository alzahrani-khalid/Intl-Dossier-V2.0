 ...
 */
export async function invalidateUserCache(redis: Redis, userId: string): Promise<number> {
...
export function validateFilters(filters: FilterCriteria): { valid: boolean; error?: string } {
  ...
  if (filters.type && !['dossier', 'ticket', 'position', 'task'].includes(filters.type)) {
    return {
      valid: false,
      error: 'Invalid type value. Must be one of: dossier, ticket, position, task',
    }
  }
  ...
  if (
    filters.sort_by &&
    !['assigned_at_asc', 'assigned_at_desc', 'priority_asc', 'priority_desc'].includes(
      filters.sort_by,
    )
  ) {
    return {
      valid: false,
      error:
        'Invalid sort_by value. Must be one of: assigned_at_asc, assigned_at_desc, priority_asc, priority_desc',
    }
  }
  ...
  if (filters.page_size !== undefined) {
    if (filters.page_size < 1 || filters.page_size > 100 || !Number.isInteger(filters.page_size)) {
      return {
        valid: false,
        error: 'Invalid page_size value. Must be an integer between 1 and 100',
      }
    }
  }
...
export function calculateAging(assignments: any[]): any[] {
  ...
  return assignments.map((assignment) => ({
    ...
    days_waiting: Math.floor(
      (now - new Date(assignment.assigned_at).getTime()) / (1000 * 60 * 60 * 24),
    ),
  }))
}
