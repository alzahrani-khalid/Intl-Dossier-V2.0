...
export class OrganizationHierarchyService {
  ...
  async getFullHierarchy(options: HierarchyOptions = {}): Promise<OrganizationNode[]> {
    const { maxDepth = 10, includeInactive = false, expandAll = false } = options

    // Fetch all organizations
    let query = this.supabase.from('organizations').select('*').order('name_en')
  ...
  private buildHierarchy(
    ...
    const children = organizations.filter((org) => org.parent_organization_id === parentId)
  ...
  async getDescendants(organizationId: string, maxDepth: number = 10): Promise<OrganizationNode[]> {
    const allOrgs = await this.getAllOrganizations()
    const descendants: OrganizationNode[] = []

    const collectDescendants = (parentId: string, level: number, path: string[]): void => {
      if (level >= maxDepth) return

      const children = allOrgs.filter((org) => org.parent_organization_id === parentId)
  ...
  async getOrganizationStats(organizationId: string): Promise<any> {
    const [descendants, { data: mous }, { data: events }] = await Promise.all([
  ...
  async searchInHierarchy(
    ...
    let query = this.supabase.from('organizations').select('*').ilike(column, `%${searchTerm}%`)
