...
export class WorkspaceService {
  ...
  async addMember(
    ...
    try {
      ...
      // Invalidate cache
      await cacheHelpers.del([`${this.cachePrefix}${workspaceId}`, `${this.cachePrefix}list:*`])
   ...
   */
  async removeMember(workspaceId: string, userId: string, removedBy: string): Promise<Workspace> {
    try {
      const workspace = await this.findById(workspaceId)
      if (!workspace) throw new Error('Workspace not found')
  ...
  async getStatistics(workspaceId: string): Promise<{
    ...
    try {
      ...
      const recentActivities = workspace.activity_feed.filter(
