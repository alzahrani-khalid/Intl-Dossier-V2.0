...
export async function findBestAssignee(workItem: WorkItem): Promise<StaffProfile | null> {
  ...
  try {
    ...
    const { data: eligibleStaff, error } = await supabase.rpc('find_eligible_staff_for_assignment', {
...
export async function attemptAssignment(
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      ...
      if (!currentStaff || currentStaff.current_assignment_count >= currentStaff.individual_wip_limit) {
