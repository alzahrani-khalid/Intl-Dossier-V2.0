...
export class CapacityService {
  ...
  async checkMultipleStaffCapacity(staffIds: string[]): Promise<Map<string, StaffCapacityStatus>> {
    ...
    const { data: staffProfiles, error } = await this.supabase
      .from('staff_profiles')
      .select(
        'user_id, individual_wip_limit, current_assignment_count, availability_status, unit_id',
      )
      .in('user_id', staffIds)
   ...
   */
  async isCapacityAtWarning(staffId: string, warningThreshold: number = 75): Promise<boolean> {
   ...
   */
  async isUnitCapacityAtWarning(unitId: string, warningThreshold: number = 75): Promise<boolean> {
  ...
  async findAvailableStaffInUnit(
    ...
    const query = this.supabase
      .from('staff_profiles')
      .select(
        'user_id, individual_wip_limit, current_assignment_count, availability_status, unit_id, skills',
      )
      .eq('unit_id', unitId)
    ...
    const capacityStatuses = filteredProfiles
      .map((staff) => {
        ...
        return {
          ...
          has_capacity,
          available_slots,
          availability_status: staff.availability_status as
            | 'available'
            | 'on_leave'
            | 'unavailable',
          unit_id: staff.unit_id,
