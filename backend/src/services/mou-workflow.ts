...
export class MoUWorkflowService {
  ...
  private initializeTransitions(): Map<string, WorkflowTransition> {
    const transitions: WorkflowTransition[] = [
      // Draft transitions
      {
        ...
        validationRules: [
          (mou) =>
            mou.primary_party_id !== mou.secondary_party_id ||
            'Primary and secondary parties must be different',
      ...
      // Internal review transitions
      {
        ...
        requiredRole: 'admin',
        validationRules: [(mou) => mou.document_version >= 1 || 'Document must be versioned'],
      },
      {
        from: 'internal_review',
        to: 'draft',
      ...
      // Negotiation transitions
      {
        ...
        validationRules: [(mou) => mou.signing_date !== null || 'Signing date is required'],
      },
      {
        from: 'negotiation',
        to: 'external_review',
      ...
      // Signed transitions
      {
        ...
        validationRules: [
          ...
          (mou) => {
            ...
            return (
              new Date(mou.expiry_date) > new Date(mou.effective_date) ||
      ...
      // Active transitions
      {
        ...
        validationRules: [
          (mou) => {
            ...
            return (
              expiryDate.getTime() - now.getTime() < gracePeriod ||
              'Can only renew within 30 days of expiry'
            )
          },
        ],
      },
      ...
      // Expired transitions
      {
        ...
        validationRules: [
          (mou) => {
            ...
            return (
              now.getTime() - expiryDate.getTime() < gracePeriod ||
  ...
  private async logTransition(request: TransitionRequest, mou: MoU): Promise<any> {
    ...
    const { data, error } = await this.supabase.from('audit_log').insert(logEntry).select().single()
