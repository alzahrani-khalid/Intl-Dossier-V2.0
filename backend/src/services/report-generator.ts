...
export class ReportGeneratorService {
  ...
  private getNextQueuedJob(): ReportJob | null {
    const queuedJobs = Array.from(this.queue.values())
      ...
      .sort((a, b) => {
        ...
        const priorityDiff =
          priorityOrder[a.request.priority || 'normal'] -
        ...
        if (a.request.scheduledAt && b.request.scheduledAt) {
          return (
            new Date(a.request.scheduledAt).getTime() - new Date(b.request.scheduledAt).getTime()
          )
        }
  ...
  private async processJob(job: ReportJob): Promise<void> {
    ...
    // Format the report
    const formattedResult = await this.formatReport(
      result,
      job.request.format,
  ...
  private async generateMouStatus(job: ReportJob): Promise<any> {
    ...
    let query = this.supabase
      .from('mous')
      .select(
        '*, primary_party:organizations!primary_party_id(*), secondary_party:organizations!secondary_party_id(*)',
      )
  ...
  private async saveReport(content: any, job: ReportJob): Promise<string> {
    ...
    await this.supabase.from('report_outputs').insert({
      job_id: job.id,
      file_name: fileName,
      file_url: mockUrl,
  ...
  private async persistJob(job: ReportJob): Promise<void> {
    await this.supabase.from('report_jobs').upsert({
      id: job.id,
