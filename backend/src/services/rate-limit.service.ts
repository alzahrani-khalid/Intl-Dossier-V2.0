...
import {
  ...
  TokenBucketState,
} from '../models/rate-limit-policy.model'
...
export class RateLimitService {
  ...
  async deletePolicy(id: string): Promise<void> {
    const { error } = await this.supabase.from('rate_limit_policies').delete().eq('id', id)
  ...
  private async saveTokenBucketState(key: string, state: TokenBucketState): Promise<void> {
    await this.redis.set(key, JSON.stringify(state), 'EX', 3600)
  }

  async getRateLimitStatus(userId?: string, ip?: string): Promise<RateLimitStatus> {
  ...
  async resetRateLimit(userId?: string, ip?: string): Promise<void> {
    const pattern = userId ? `rate_limit:user:${userId}:*` : `rate_limit:ip:${ip}:*`
  ...
  async getAllPolicies(appliesTo?: RateLimitPolicy['applies_to']): Promise<RateLimitPolicy[]> {
    let query = this.supabase.from('rate_limit_policies').select('*')
  ...
  async applyBurstLimit(burst: number, userId?: string, ip?: string): Promise<boolean> {
