...
export class MFAService {
  async enroll(token: Token, factorType: string): Promise<MFAEnrollment> {
    if (enrolled.has(token)) {
      throw Object.assign(new Error('MFA already enrolled'), {
        code: 'MFA_ALREADY_ENROLLED',
        status: 409,
      })
    }
    if (!factorType) {
      throw Object.assign(new Error('Missing factor_type'), {
        code: 'MISSING_FACTOR_TYPE',
        status: 400,
      })
    }
    if (factorType !== 'totp') {
      throw Object.assign(new Error('Invalid factor_type'), {
  ...
  verifyCode(token: Token, code: string, factorId?: string) {
    ...
    if (!/^[0-9]{6}$/.test(code || '')) {
      throw Object.assign(new Error('Invalid code format'), {
  ...
  recoverWithBackupCode(token: Token, backupCode: string) {
    ...
    if (!backupCode) {
      throw Object.assign(new Error('Missing backup_code'), {
        code: 'MISSING_BACKUP_CODE',
        status: 400,
      })
    }
    if (!/^[A-Z0-9]{8}$/i.test(backupCode)) {
      throw Object.assign(new Error('Invalid backup code format'), {
        code: 'INVALID_BACKUP_CODE_FORMAT',
        status: 400,
      })
    }
    ...
    if (used.has(normalized)) {
      throw Object.assign(new Error('Backup code already used'), {
    ...
    if (!entry.backupCodes.includes(normalized)) {
      ...
      throw Object.assign(new Error('Invalid backup code'), {
