...
async function executeJob(): Promise<void> {
  ...
  try {
    ...
    if (!lockAcquired) {
      console.warn('[OVERDUE-CHECK] Job already running, skipping')
      return
    }

    console.warn('[OVERDUE-CHECK] Starting daily overdue commitment detection')

    // Call detect-overdue-commitments Edge Function
    const response = await fetch(`${SUPABASE_URL}/functions/v1/detect-overdue-commitments`, {
      method: 'POST',
      headers: {
    ...
    console.warn(`[OVERDUE-CHECK] Marked ${result.overdueCount} commitments as overdue`)
    console.warn(`[OVERDUE-CHECK] Sent ${result.notificationsSent} notifications`)
    console.warn(
      `[OVERDUE-CHECK] Triggered health recalculation for ${result.healthScoresRecalculated} dossiers`,
    )
    console.warn(`[OVERDUE-CHECK] Completed in ${result.executionTimeMs}ms`)
  } catch (error) {
...
export function scheduleOverdueCommitmentsDetectionJob(): void {
  ...
  console.warn('[OVERDUE-CHECK] Scheduled job registered: daily at 2:00 AM')
}
