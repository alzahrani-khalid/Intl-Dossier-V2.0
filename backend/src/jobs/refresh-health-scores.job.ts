...
async function refreshMaterializedViews(): Promise<void> {
  const response = await fetch(`${SUPABASE_URL}/functions/v1/refresh-commitment-stats`, {
    method: 'POST',
    headers: {
...
async function recalculateStaleHealthScores(): Promise<number> {
  ...
  const staleDossierIds = staleScores.map((score) => score.dossier_id)

  if (staleDossierIds.length === 0) {
    console.warn('[HEALTH-REFRESH] No stale health scores found')
    return 0
  }

  // Trigger health recalculation
  const recalcResponse = await fetch(`${SUPABASE_URL}/functions/v1/trigger-health-recalculation`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      Authorization: `Bearer ${SUPABASE_SERVICE_ROLE_KEY}`,
        ...
        if (newOverallScore < 60 && previousOverallScore >= 60) {
          try {
            ...
            if (recalcResult.components.engagementFrequency < previousScore.engagement_frequency) {
              const diff =
                previousScore.engagement_frequency - recalcResult.components.engagementFrequency
              factors.push(`engagement frequency decreased by ${diff} points`)
            }

            if (
              recalcResult.components.commitmentFulfillment < previousScore.commitment_fulfillment
            ) {
              const diff =
                previousScore.commitment_fulfillment - recalcResult.components.commitmentFulfillment
            ...
            console.error(
              `[HEALTH-REFRESH] Failed to send notification for dossier ${recalcResult.dossierId}:`,
              error,
            )
  ...
  console.warn(`[HEALTH-REFRESH] Recalculated ${result.dossierCount} stale health scores`)
...
async function executeJob(): Promise<void> {
  ...
  try {
    ...
    const lockAcquired = await redis.set(lockKey, '1', 'EX', lockExpiration, 'NX')

    if (!lockAcquired) {
      console.warn('[HEALTH-REFRESH] Job already running, skipping')
      return
    }

    console.warn('[HEALTH-REFRESH] Starting')
    ...
    console.warn(`[HEALTH-REFRESH] Completed: ${recalculatedCount} health scores recalculated`)
  } catch (error) {
...
export function scheduleHealthScoresRefreshJob(): void {
  ...
  console.warn('[HEALTH-REFRESH] Scheduled job registered: every 15 minutes')
}
