...
export class PositionConsistencyModel {
  ...
  static calculateConsistencyScore(totalPositions: number, conflicts: PositionConflict[]): number {
    if (totalPositions === 0) {
      return 100
    }
  ...
  static generateRecommendations(conflicts: PositionConflict[]): string[] {
    ...
    if (outdatedCount > 0) {
      recommendations.push(`Update ${outdatedCount} outdated positions to reflect current policy`)
    }

    const ambiguityCount = conflicts.filter((c) => c.conflict_type === 'ambiguity').length
    if (ambiguityCount > 0) {
      recommendations.push(`Clarify ${ambiguityCount} ambiguous positions to improve clarity`)
    }
