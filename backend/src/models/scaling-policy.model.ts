...
export class ScalingPolicyModel {
  ...
  static shouldScaleUp(policy: ScalingPolicy, metrics: ScalingMetrics): boolean {
    ...
    const durationMet =
      metrics.threshold_exceeded_duration_minutes >= policy.threshold_duration_minutes
  ...
  static calculateDesiredInstances(policy: ScalingPolicy, metrics: ScalingMetrics): number {
    ...
    return Math.max(policy.min_instances, Math.min(policy.max_instances, desiredInstances))
  }
  ...
  static getScalingDecision(
    ...
    action: 'scale_up' | 'scale_down' | 'maintain' | 'degrade'
