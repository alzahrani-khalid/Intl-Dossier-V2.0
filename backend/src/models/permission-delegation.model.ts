...
class PermissionDelegationModel {
  ...
  static isActive(delegation: PermissionDelegation): boolean {
    ...
    const now = new Date()
    return now >= new Date(delegation.valid_from) && now <= new Date(delegation.valid_until)
  }

  static hasPermission(delegation: PermissionDelegation, permission: PermissionType): boolean {
