...
describe('RLS Policies - Briefs', () => {
  ...
  it('should be immutable (no updates)', async () => {
    ...
    const { data: brief } = await supabaseService.from('briefs').select('id').limit(1).single()
...
describe('Data Integrity', () => {
  it('should validate dossier type enum', async () => {
    const { error } = await supabaseService.from('dossiers').insert({
  ...
  it('should validate email format in key_contacts', async () => {
    const { error } = await supabaseService.from('key_contacts').insert({
      dossier_id: TEST_DOSSIER_LOW_SENSITIVITY,
      name: 'Test Contact',
  ...
  it('should validate brief content structure (JSONB)', async () => {
    const { error } = await supabaseService.from('briefs').insert({
      dossier_id: TEST_DOSSIER_LOW_SENSITIVITY,
