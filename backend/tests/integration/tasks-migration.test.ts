...
describe('Tasks Migration Integrity Tests', () => {
  ...
  describe('Count Validation (No Data Loss)', () => {
    ...
    it('should have no invalid status values after migration', async () => {
      ...
      const { data: tasks, error } = await supabase.from('tasks').select('id, status')
  ...
  describe('Sample Data Comparison', () => {
    it('should preserve all critical fields from assignments', async () => {
      // Fetch sample tasks
      const { data: tasks, error } = await supabase.from('tasks').select('*').limit(10)
    ...
    it('should have generated default titles for tasks without explicit titles', async () => {
      const { data: tasks, error } = await supabase.from('tasks').select('title').limit(100)
      ...
      const autoGeneratedTitles = tasks.filter(
        (task) => task.title.startsWith('Assignment #') || task.title.startsWith('Task #'),
      )
  ...
  describe('Referential Integrity', () => {
    it('should maintain foreign key relationships to auth.users', async () => {
      const { data: tasks, error } = await supabase.from('tasks').select('assignee_id').limit(100)
    ...
    it('should maintain foreign key relationships to engagements', async () => {
      ...
      const engagementIds = [
        ...new Set(tasks.map((t) => t.engagement_id).filter(Boolean) as string[]),
      ]
