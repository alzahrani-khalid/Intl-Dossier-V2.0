-- Fix enhanced SLA monitoring 42703: the staff_profiles table has no full-name
-- columns (cols: id, user_id, unit_id, role, ...). The SLA assignee/at-risk RPCs and the
-- sla_compliance_by_assignee view in 20260111600001_enhanced_sla_monitoring.sql selected
-- staff-profile full-name / full-name-ar fields that do not exist, which would fail at
-- apply with 42703 undefined_column. Because of that, 20260111600001 was NEVER applied —
-- its 4 dashboard RPCs and capture_sla_daily_snapshot are absent from live staging and
-- from the generated types.
-- Fix: join the users table on the staff-profile user_id and select the user full_name /
-- name_ar columns (users.full_name exists, users.name_ar exists; note the users table has
-- no Arabic full-name column, so name_ar is the correct one). All other logic is preserved
-- byte-for-byte (sla #2, sla #4). Applied 2026-06-10.
...
WHERE t.assigned_to IS NOT NULL
GROUP BY t.assigned_to, sp.id, u.full_name, u.name_ar, sp.unit_id;
...
BEGIN
  ...
  GROUP BY t.assigned_to, u.full_name, u.name_ar
  ORDER BY COUNT(DISTINCT t.id) DESC
  LIMIT p_limit;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
