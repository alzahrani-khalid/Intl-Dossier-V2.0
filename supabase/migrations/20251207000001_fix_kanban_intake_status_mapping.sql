-- Migration: Fix Kanban Intake Ticket Status Mapping
-- Feature: 034-unified-kanban
-- Issue: Status mapping used non-existent 'resolved' and 'pending' statuses
-- 
-- The ticket_status enum is:
--   ('draft', 'submitted', 'triaged', 'assigned', 'in_progress', 'converted', 'closed', 'merged')
-- 
-- This migration corrects the intake ticket column_key mapping in the
-- get_unified_work_kanban RPC function to use valid statuses.

BEGIN;

-- ============================================
-- 1. Drop and recreate the Kanban RPC function with corrected status mapping
-- ============================================
...
BEGIN
  ...
  WITH base_items AS (
    ...
    -- INTAKE_TICKETS
    -- FIXED: Use actual ticket_status enum values
    ...
    SELECT
      ...
      CASE p_column_mode
        WHEN 'status' THEN
          CASE i.status::TEXT
            -- Terminal/completed states -> 'done' column
            WHEN 'converted' THEN 'done'
            -- Cancelled/closed states -> 'cancelled' column
...
-- Update function comment to document the status mapping
COMMENT ON FUNCTION get_unified_work_kanban IS
'Returns unified work items (tasks, commitments, intake tickets) grouped by column key for Kanban board display.

Supports:
- Context types: personal, dossier, engagement
- Column modes: status, priority, tracking_type

Intake ticket status mapping (ticket_status enum -> kanban column):
- draft, submitted, triaged -> todo
- assigned, in_progress -> in_progress
- converted -> done
- closed, merged -> cancelled

This function was updated on 2025-12-07 to fix invalid status references.';
...
-- ============================================
-- To rollback this migration, restore the original function from:
-- 20251203000002_create_kanban_rpc_function.sql
-- 
