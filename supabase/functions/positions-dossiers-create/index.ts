...
serve(async (req) => {
  ...
  try {
    ...
    if (!dossier_id) {
      return new Response(JSON.stringify({ error: 'Missing required field: dossier_id' }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      })
    }
    ...
    const { data: link, error: insertError } = await supabaseClient
      ...
      .insert({
        ...
        dossier_id,
        // live position_dossier_links_link_type_check allows
        // applies_to | related_to | endorsed_by | opposed_by (verified 2026-06-10).
        // The old default 'related' violated the CHECK and 500'd every attach.
        link_type: link_type || 'related_to',
        notes,
      ...
      .select(
        `
        *,
        dossier:dossiers!dossier_id(id, name_en, name_ar, type, status)
      `,
      )
      .single()
