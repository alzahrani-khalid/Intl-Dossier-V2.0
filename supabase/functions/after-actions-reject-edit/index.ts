...
serve(async (req) => {
  ...
  try {
    ...
    // Check user role from public.users (must be supervisor or admin)
    ...
    if (
      ...
      body.rejection_reason.length < 10 ||
      body.rejection_reason.length > 500
    ) {
      return new Response(
        JSON.stringify({
    ...
    if (fetchError || !currentRecord) {
      return new Response(JSON.stringify({ error: 'After-action record not found' }), {
        status: 404,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      })
    }
    ...
    const { error: versionError } = await supabaseClient.from('after_action_versions').insert({
