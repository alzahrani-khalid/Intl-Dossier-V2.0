...
serve(async (req) => {
  ...
  try {
    ...
    if (!['automatic_quarterly', 'manual_override', 'disable'].includes(body.schedule_type)) {
      return new Response(
        JSON.stringify({
          error:
    ...
    if (body.schedule_type === 'automatic_quarterly') {
      ...
      const cronCommand = `
        ...
        SELECT
          ...
          NULL,
          'pending',
          ${reviewerId ? `'${reviewerId}'` : "(SELECT id FROM public.users WHERE role = 'admin' ORDER BY created_at ASC LIMIT 1)"},
          now()
        WHERE NOT EXISTS (
      ...
      const { error: createError } = await supabaseAdmin.from('access_reviews').insert({
