  ...
  try {
    ...
    const refreshPromises = typesToRefresh.map(async (intelligenceType) => {
      try {
        ...
        const { data: upsertedData, error: updateError } = await serviceClient
          .from('intelligence_reports')
          .upsert({
            entity_id,
        ...
        console.log(`[${intelligenceType.toUpperCase()}] Upserted data - Has metrics: ${!!upsertedData?.[0]?.metrics}, Metrics:`, upsertedData?.[0]?.metrics);
