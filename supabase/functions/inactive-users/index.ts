...
serve(async (req) => {
  ...
  try {
    ...
    const inactiveDays = inactiveDaysParam ? parseInt(inactiveDaysParam, 10) : 90
    ...
    const { data: inactiveUsersData, error: queryError } = await supabaseAdmin
      ...
      .or(`last_login_at.is.null,last_login_at.lt.${thresholdDate.toISOString()}`)
      .eq('is_active', true) // Only active accounts that are inactive
      .limit(limit)
