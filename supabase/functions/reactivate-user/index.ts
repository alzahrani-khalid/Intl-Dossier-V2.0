...
serve(async (req) => {
  ...
  try {
    ...
    if (targetUser.is_active !== false) {
      return new Response(JSON.stringify({ success: false, error: 'User is not deactivated' }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      })
    }
    ...
    if (reactivateError) {
      ...
      return new Response(JSON.stringify({ success: false, error: 'Failed to reactivate user' }), {
        status: 500,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      })
    }
