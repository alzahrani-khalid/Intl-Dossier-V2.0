...
async function fetchEntityData(
  ...
  const { data, error } = await supabase.from(table).select('id, created_at, updated_at').eq('id', entityId).single();
...
async function fetchContextData(
  ...
  if (shouldFetch('activity')) {
    let activityQuery = supabase
      ...
      .select('id, dossier_id, event_type, event_title_en, event_title_ar, event_description_en, event_description_ar, event_date, created_at, updated_at')
  ...
  if (shouldFetch('commitments')) {
    const { data: commitments } = await supabase
      .from('commitments')
      .select('id, dossier_id, description_en, description_ar, status, deadline, created_at, updated_at')
  ...
  if (shouldFetch('activity') || shouldFetch('strategic')) {
    const { data: engagements } = await supabase
      .from('engagements')
      .select('id, dossier_id, title_en, title_ar, description_en, description_ar, engagement_date, status, created_at, updated_at')
      .eq('dossier_id', entityId)
