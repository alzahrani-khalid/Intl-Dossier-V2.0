...
// Resolve the user's organization id through a cascade of authoritative sources.
// The briefs RLS org claim (auth.jwt() ->> 'org_id') is not populated for users
// in this deployment, so the JWT alone never yields an org. Order:
//   1) JWT 'org_id' claim / app_metadata  (kept first — multi-tenant-ready)
//   2) public.users.default_organization_id (by auth user id)
//   3) public.profiles.organization_id     (keyed by user_id)
//   4) the sole public.organizations row   (single-tenant fallback)
// Uses the service-role client (user + engagement access already verified).
async function resolveOrgId(
  admin: ReturnType<typeof createClient>,
  token: string,
  user: { id: string; app_metadata?: Record<string, unknown> },
): Promise<string | null> {
  const claimOrg = decodeOrgId(token, user)
  ...
  const { data: userRow } = await admin
    ...
    .eq('id', user.id)
    .maybeSingle()
  const userOrg = (userRow as { default_organization_id?: string } | null)?.default_organization_id
  if (typeof userOrg === 'string' && userOrg) return userOrg
  ...
  const { data: profileRow } = await admin
    ...
    .eq('user_id', user.id)
    .maybeSingle()
  const profileOrg = (profileRow as { organization_id?: string } | null)?.organization_id
  if (typeof profileOrg === 'string' && profileOrg) return profileOrg

  const { data: orgs } = await admin.from('organizations').select('id').limit(2)
  if (Array.isArray(orgs) && orgs.length === 1) {
    const onlyOrg = (orgs[0] as { id?: string }).id
    if (typeof onlyOrg === 'string' && onlyOrg) return onlyOrg
  }
...
async function handleCreateManualBrief(
  ...
  // briefs.tenant_id is NOT NULL and briefs RLS keys on organization_id. Resolve
  // both from authoritative sources (JWT claim is empty for all users here), via
  // the service-role client. tenant_id mirrors organization_id (org-isolation model).
  const orgId = await resolveOrgId(admin, token, user)
  if (!orgId) {
    return errorResponse(
      'NO_ORG',
  ...
  return new Response(
    JSON.stringify({ data: { ...inserted, brief_type: 'legacy', source: 'manual' } }),
    {
      status: 201,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    },
...
function buildContent(
  ...
  if (ctx.host_country || ctx.host_organization) {
    lines.push(`## ${T.host}`)
    if (ctx.host_country)
      lines.push(`- ${T.hostCountry}: ${pick(ctx.host_country.name_en, ctx.host_country.name_ar)}`)
    if (ctx.host_organization)
      lines.push(
        `- ${T.hostOrg}: ${pick(ctx.host_organization.name_en, ctx.host_organization.name_ar)}`,
      )
    lines.push('')
  }
