...
#### [Pattern] Created feature-specific i18n files (email-digest.json) rather than adding keys to global translation files (2026-01-15)

- **Problem solved:** Email digest settings page needed ~40 new translation keys for UI labels, help text, and schedules
- **Why this works:** Keeps translation files scoped and maintainable; easier to iterate on feature-specific translations; prevents global translation file bloat
- **Trade-offs:** Requires importing feature-specific translation namespace in component; slightly more boilerplate but better organization

#### [Pattern] Separated i18n translations into namespace files (commitment-deliverables.json) and explicitly registered namespace in index.ts rather than flattening into single large JSON (2026-01-15)

- **Problem solved:** Managing translations for new deliverables feature alongside existing commitment translations
- **Why this works:** Namespace separation keeps feature translations isolated and maintainable. Feature team can work independently on translations without merge conflicts. Makes it obvious which translations belong to which feature. Explicit registration in index.ts provides visibility into all registered namespaces
- **Trade-offs:** Easier: Modular, scalable, independent work. Harder: Requires namespace lookup in components, more files to manage

#### [Pattern] Used language-specific nested keys (agenda.builder.title, agenda.items.empty_state) instead of flat keys (2026-01-15)

- **Problem solved:** Meeting agenda has multiple UI sections (builder, items, timing, participants, documents) each with multiple strings
- **Why this works:** Enables translation teams to work on sections independently; easier to find related strings; scales better as features add
- **Trade-offs:** Slightly deeper i18n calls but much better organization; duplication prevention easier with nested structure
