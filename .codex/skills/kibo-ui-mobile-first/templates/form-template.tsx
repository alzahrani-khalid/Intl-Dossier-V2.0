 ...
 * - RTL-compatible
 */
export function ResponsiveFormTemplate({ onSubmit, defaultValues }: ResponsiveFormTemplateProps) {
  ...
  return (
    <Form {...form}>
      ...
      >
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <Select onValueChange={field.onChange} defaultValue={field.value}>
                <FormControl>
                  <SelectTrigger className={cn('min-h-[44px]', 'sm:min-h-[40px]')}>
        ...
        {/* Form Actions */}
        <div className={cn('flex gap-2 pt-2', isMobile ? 'flex-col' : 'flex-row justify-end')}>
          <Button
            type="button"
            variant="outline"
            onClick={() => form.reset()}
            className={cn('min-h-[44px]', 'sm:min-h-[40px]', isMobile && 'w-full')}
          >
            {t('actions.cancel')}
          </Button>
          <Button
